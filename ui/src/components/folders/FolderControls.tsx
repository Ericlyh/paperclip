...
function selectionLabel({
  ...
  if (selection === "unfiled") return t("folders.unfiled");
...
export function FolderChip({
  ...
  return (
    <Button variant="outline" size="sm" className="max-w-full justify-start" onClick={onClick}>
      ...
      <span className="truncate">{selectionLabel({ folders: result?.folders ?? [], selection, allLabel, t })}</span>
  ...
  return (
    <nav aria-label={`${itemLabelPlural} folders`} className="hidden w-(--sz-folder-rail) shrink-0 border-r border-border pr-3 md:block">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("folders.railHeading")}</div>
        <Button variant="ghost" size="icon-sm" title={t("folders.newFolder")} onClick={onCreate}>
      ...
      {loading ? (
        ...
        <div className="space-y-0.5">
          ...
          {renderVirtualRow("unfiled", t("folders.unfiled"), result?.unfiledCount ?? 0, <FolderSwatch color={null} className="mt-0.5" />)}
        </div>
      )}
    </nav>
  );
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("folders.folderActionsFor", { name: folder.name })}
          >
            <MoreHorizontal className="h-3.5 w-3.5" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end">
          <DropdownMenuItem onSelect={onStartRename}>{t("folders.rename")}</DropdownMenuItem>
          <DropdownMenuItem onSelect={onEdit}>{t("folders.editColor")}</DropdownMenuItem>
          <DropdownMenuSeparator />
  ...
  return (
    <div className="mb-3 flex flex-wrap items-center gap-2 rounded-md border border-border bg-muted/40 px-3 py-2 text-sm">
      ...
      <span className="min-w-0 flex-1 text-muted-foreground">
        {t("folders.groupThese", { itemLabelPlural })}
      </span>
      <Button size="sm" variant="outline" onClick={onCreateFolder}>
        {t("folders.createFirstFolder")}
      </Button>
      <Button size="icon-sm" variant="ghost" aria-label={t("folders.dismissSuggestion")} onClick={dismiss}>
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-(--sz-folder-sheet-max) rounded-t-lg pb-4">
        <SheetHeader className="border-b border-border px-4 py-3">
          <SheetTitle>{t("folders.sheetTitle", { itemLabelPlural })}</SheetTitle>
        </SheetHeader>
        <div className="overflow-y-auto px-3">
          ...
          {result?.kind === "skill" ? (
            <>
              {model.my ? renderBranch(model.my, t("folders.mySkills")) : null}
              ...
              {model.projects ? renderBranch(model.projects, t("folders.projects")) : null}
              {model.bundled ? renderBranch(model.bundled, t("folders.bundled")) : null}
            </>
          ) : (
...
export function MoveToMenu({
  ...
  return (
    <DropdownMenuSub>
      <DropdownMenuSubTrigger>{t("folders.moveToEllipsis")}</DropdownMenuSubTrigger>
  ...
  return (
    <>
        ...
        {visibleFolders.length === 0 ? (
          <div className="px-2 py-2 text-xs text-muted-foreground">{t("folders.noFoldersMatch")}</div>
        ) : null}
        <DropdownMenuSeparator />
        <DropdownMenuItem onSelect={onCreateAndMove}>
          <Plus className="h-3.5 w-3.5" />
          {t("folders.newFolderEllipsis")}
        </DropdownMenuItem>
    </>
  );
}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{isEdit ? t("folders.editFolder") : t("folders.createFolder")}</DialogTitle>
          <DialogDescription>
            {kind === "routine" ? t("folders.organizeRoutines") : t("folders.organizeSkills")}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-medium" htmlFor="folder-name">{t("folders.name")}</label>
            <Input
              id="folder-name"
              value={name}
          ...
          <div className="space-y-2">
            <div className="text-sm font-medium">{t("folders.color")}</div>
            <div className="flex flex-wrap gap-2">
              {FOLDER_COLORS.map((swatch) => (
                <button
                  key={swatch}
                  type="button"
                  aria-label={t("folders.useFolderColor", { color: swatch })}
                  className={cn(
        ...
        <DialogFooter>
          ...
          <Button onClick={() => onSubmit({ name: name.trim(), color })} disabled={pending || !name.trim()}>
            {pending ? t("folders.savingEllipsis") : isEdit ? t("folders.save") : t("folders.createFolder")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          ...
          >
            {pending ? t("folders.deletingEllipsis") : t("folders.deleteFolder")}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
...
export function BulkBar({
  ...
  return (
    <div className="sticky top-2 z-10 flex flex-wrap items-center gap-2 rounded-md border border-border bg-background/95 px-3 py-2 shadow-sm backdrop-blur">
      <span className="mr-auto text-sm text-muted-foreground">{t("folders.countSelected", { count: selectedCount })}</span>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button size="sm" variant="outline">{t("folders.moveToEllipsis")}</Button>
        </DropdownMenuTrigger>
      ...
      <Button size="sm" variant="ghost" onClick={onClear}>{t("folders.deselectAll")}</Button>
      <Button size="sm" onClick={onDone}>{t("folders.done")}</Button>
    </div>
  );
}
