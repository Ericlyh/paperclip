...
export function SkillFolderRail({
  ...
  return (
    <div className="relative hidden h-full shrink-0 md:flex" style={{ width: `${width}px` }}>
      <nav
        aria-label={t("folders.aria.skillFolders")}
      ...
      <div className="mb-2 flex items-center justify-between gap-2 pt-0.5">
        <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
          {t("skills.scanner.folders")}
        </div>
        <Button variant="ghost" size="icon-sm" title={t("skills.scanner.folderNew")} onClick={() => onCreateFolder(null)}>
      ...
      {loading ? (
        ...
        <div className="space-y-0.5">
          <VirtualRow
            ...
            label={t("skills.scanner.allSkills")}
            count={allCount}
          ...
          {model.my ? (
            <TreeBranch
              ...
              rootLabel={t("skills.scanner.mySkills")}
            ...
            <VirtualRow
              active={false}
              label={t("skills.scanner.mySkills")}
              count={0}
          ...
          <RailHeading label={t("skills.scanner.company")} onCreate={() => onCreateFolder(null)} />
          {model.company.length > 0 ? (
            ...
            <div className="px-2 py-1 text-xs text-muted-foreground">{t("skills.scanner.noCompanyFolders")}</div>
          )}
          ...
          {model.projects ? (
            <TreeBranch
              ...
              renamingId={renamingId}
              renameDraft={renameDraft}
              rootLabel={t("skills.scanner.projects")}
            ...
            <VirtualRow
              active={false}
              label={t("skills.scanner.projects")}
              count={0}
          ...
          {model.bundled ? (
            <TreeBranch
              ...
              renamingId={renamingId}
              renameDraft={renameDraft}
              rootLabel={t("skills.scanner.bundled")}
              rootIcon={<Boxes className="h-3.5 w-3.5" />}
              onToggle={toggle}
              onSelect={onSelect}
              onCreateFolder={onCreateFolder}
          ...
          <VirtualRow
            ...
            label={t("skills.scanner.unfiled")}
            count={unfiledCount}
      ...
      <div
        role="separator"
        aria-label={t("folders.aria.resizeSkillFolders")}
  ...
  return (
    <div className="group/heading flex items-center justify-between px-2 pb-0.5 pt-3">
      ...
      <button
        type="button"
        onClick={onCreate}
        title={t("skills.scanner.newXFolder", { name: label.toLowerCase() })}
  ...
  return (
    <div>
      ...
      >
        <button
          type="button"
          aria-label={isOpen ? t("skills.scanner.collapseFolder") : t("skills.scanner.expandFolder")}
          className={cn(
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                aria-label={t("skills.scanner.folderActionsFor", { name: label })}
              >
            ...
            <DropdownMenuContent align="end">
              {canNest ? (
                <DropdownMenuItem onSelect={() => onCreateFolder(folder.id)}>
                  ...
                  {t("skills.scanner.newSubfolder")}
                </DropdownMenuItem>
              ) : null}
              {editable ? (
                <>
                  <DropdownMenuItem onSelect={() => onStartRename(folder)}>{t("common.rename")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onEditFolder(folder)}>{t("skills.scanner.editColor")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onMoveFolder(folder, isInMySkills ? "company" : "my")}>
                    <MoveRight className="h-3.5 w-3.5" />
                    {isInMySkills ? t("skills.scanner.moveToCompany") : t("skills.scanner.moveToMySkills")}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
...
export function FolderBreadcrumb({
  ...
  return (
    <nav aria-label={t("folders.aria.folderPath")} className="flex flex-wrap items-center gap-1 text-sm">
      <button
        type="button"
      ...
      >
        ...
        {t("skills.scanner.allSkills")}
      </button>
      {selection === "unfiled" ? (
        <>
          <ChevronRight className="h-3.5 w-3.5 text-muted-foreground/60" />
          <span className="rounded px-1.5 py-0.5 font-medium text-foreground">{t("skills.scanner.unfiled")}</span>
        </>
      ) : null}
...
export function MoveToFolderDialog({
  ...
  function renderNode(node: FolderTreeNode, depth: number, t: TFunction): ReactNode {
    const { folder } = node;
    ...
    return (
      <div key={folder.id}>
        ...
        >
          ...
          {isCurrent ? <span className="text-xs text-muted-foreground">{t("common.current")}</span> : null}
          {bundled ? <span className="text-xs text-muted-foreground">{t("common.readOnly")}</span> : null}
          {isChosen ? <Check className="h-3.5 w-3.5" /> : null}
          {nestable ? (
            <button
              type="button"
              title={t("skills.scanner.folderNewInside")}
        ...
        {visibleChildren.map((child) => renderNode(child, depth + 1, t))}
      </div>
    );
  }
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="flex items-center gap-2 rounded-md border border-border px-2.5">
          ...
          <input
            ...
            placeholder={t("skills.scanner.searchFolders")}
        ...
        <div className="max-h-72 overflow-y-auto rounded-md border border-border p-1">
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate">{t("skills.scanner.unfiled")}</span>
            {currentFolderId == null ? <span className="text-xs text-muted-foreground">{t("common.current")}</span> : null}
            {chosen === null ? <Check className="h-3.5 w-3.5" /> : null}
          </div>
          {model.roots.map((node) => renderNode(node, 0, t))}
          <div className="mt-1 border-t border-border pt-1">
            {creatingParent === null ? (
              ...
              >
                <Plus className="h-3.5 w-3.5" />
                {t("skills.scanner.newTopLevelFolder")}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="min-h-5 text-xs text-muted-foreground">
          {previewPath ? (
            <span>
              {t("skills.scanner.movingTo", { path: previewPath })}
            </span>
          ) : (
            <span>{t("skills.scanner.pickDestinationFolder")}</span>
          )}
        </div>
        ...
        <DialogFooter>
          ...
          >
            {pending ? t("common.moving") : t("skills.scanner.moveHere")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  return (
    <div className="flex items-center gap-2 py-1" style={{ paddingLeft: `${0.5 + depth * 0.9}rem` }}>
      ...
      <Input
        ...
        placeholder={t("skills.scanner.folderName")}
        autoFocus
