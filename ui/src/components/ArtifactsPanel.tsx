...
const FILTERS: Array<{ labelKey: string; value: FilterValue }> = [
  ...
  { labelKey: "status.inProgress", value: "in_progress" },
  { labelKey: "status.forReview", value: "for_review" },
...
function statusBadge(status: string): { labelKey: string | null; label: string; className: string } {
  switch (status) {
    case "active":
    case "draft":
      return { labelKey: "status.inProgress", label: status, className: "bg-blue-500/10 text-blue-600 dark:text-blue-400" };
    case "ready_for_review":
      return { labelKey: "status.forReview", label: status, className: "bg-amber-500/10 text-amber-600 dark:text-amber-400" };
    case "approved":
    case "merged":
      return { labelKey: "status.completed", label: status, className: "bg-green-500/10 text-green-600 dark:text-green-400" };
    case "changes_requested":
      return { labelKey: "status.changesRequested", label: status, className: "bg-orange-500/10 text-orange-600 dark:text-orange-400" };
    case "failed":
      return { labelKey: "status.failed", label: status, className: "bg-red-500/10 text-red-600 dark:text-red-400" };
    default:
      return { labelKey: null, label: status, className: "bg-muted text-muted-foreground" };
  }
}
  ...
  return (
    <div className="flex flex-col h-full" data-artifacts-panel>
      <div className="px-4 py-3 border-b border-border flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold">{t("artifacts.panelTitle")}</h3>
      </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-auto-hide p-4">
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">{t("artifacts.documentNotAvailable")}</p>
        ) : doc?.body ? (
          <div className="prose prose-sm dark:prose-invert max-w-none">
            <MarkdownBody>{doc.body}</MarkdownBody>
          </div>
        ) : (
          <p className="text-sm text-muted-foreground">{t("artifacts.documentEmpty")}</p>
        )}
      </div>
      ...
      {needsAction && (
        <div className="border-t border-border px-4 py-3 bg-background shrink-0">
          <p className="text-(length:--text-micro) text-muted-foreground mb-2">{t("artifacts.needsYourReview")}</p>
        ...
        <div className="border-t border-orange-500/30 bg-orange-500/5 px-4 py-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-(length:--text-compact) font-medium text-orange-700 dark:text-orange-400">
              {t("artifacts.changesRequestedBanner")}
            </p>
          </div>
        </div>
      )}
