  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("agentProperties.label.status")}>
          <AgentStatusBadge status={agent.status} />
        </PropertyRow>
        {lastErrorIsActive && agent.errorReason && (
          <PropertyRow label={t("agentProperties.label.errorReason")}>
            <span className="text-xs text-red-600 dark:text-red-400 break-words min-w-0">
              {agent.errorReason}
            </span>
          </PropertyRow>
        )}
        <PropertyRow label={t("agentProperties.label.role")}>
          <span className="text-sm">{roleLabels[agent.role] ?? agent.role}</span>
        </PropertyRow>
        {agent.title && (
          <PropertyRow label={t("agentProperties.label.title")}>
            <span className="text-sm">{agent.title}</span>
          </PropertyRow>
        )}
        <PropertyRow label={t("agentProperties.label.adapter")}>
      ...
      <div className="space-y-1">
        {(runtimeState?.sessionDisplayId ?? runtimeState?.sessionId) && (
          <PropertyRow label={t("agentProperties.label.session")}>
        ...
        {runtimeState?.lastError && (
          <PropertyRow label={t(lastErrorIsActive ? "agentProperties.label.lastError" : "agentProperties.label.lastRunError")}>
            <span
              className={
                lastErrorIsActive
        ...
        {agent.lastHeartbeatAt && (
          <PropertyRow label={t("agentProperties.label.lastHeartbeat")}>
            <span className="text-sm">{formatDate(agent.lastHeartbeatAt)}</span>
          </PropertyRow>
        )}
        {agent.reportsTo && (
          <PropertyRow label={t("agentProperties.label.reportsTo")}>
            {reportsToAgent ? (
        ...
        <PropertyRow label={t("agentProperties.label.created")}>
