...
function IssueChatLiveRunStatusLine({
  ...
  const primary =
    currentToolName
      ? t("chatThread.liveRun.usingTool", { tool: currentToolName })
  ...
  const activityText = lastActivityElapsed
    ? lastActivityAgeMs !== null && lastActivityAgeMs >= 15_000
      ...
      : t("chatThread.liveRun.ago", { elapsed: lastActivityElapsed })
    : "";
...
function IssueAssigneePausedNotice({ agent }: { agent: Agent | null }) {
  ...
  const pauseDetail =
    agent.pauseReason === "budget"
      ? t("chatThread.pausedNotice.budgetPaused")
      : agent.pauseReason === "system"
        ? t("chatThread.pausedNotice.systemPaused")
        : t("chatThread.pausedNotice.manuallyPaused");
    ...
    <div className="mb-3 rounded-md border border-orange-300/70 bg-orange-50/90 px-3 py-2.5 text-sm text-orange-950 shadow-sm dark:border-orange-500/40 dark:bg-orange-500/10 dark:text-orange-100">
      <div className="flex items-start gap-2">
        ...
        <p className="min-w-0 leading-5">
          {t("chatThread.pausedNotice.isPaused", { name: agent.name, detail: pauseDetail })}
        </p>
      </div>
    </div>
  );
...
function fallbackAuthorLabel(message: ThreadMessage, t: (key: string) => string) {
  ...
  if (message.role === "assistant") return t("chatThread.fallback.authorAgent");
  if (message.role === "user") return t("chatThread.fallback.authorUser");
  return t("chatThread.fallback.authorSystem");
}
      ...
      <div className="rounded-xl border border-amber-300/60 bg-amber-50/80 px-4 py-3 text-sm text-amber-900 dark:border-amber-500/30 dark:bg-amber-950/20 dark:text-amber-200">
        <div className="flex items-start gap-2">
          ...
          <div className="space-y-1">
            <p className="font-medium">{t("chatThread.fallback.internalError")}</p>
            <p className="text-xs opacity-80">
              {t("chatThread.fallback.internalErrorDetail")}
            </p>
          </div>
        </div>
      </div>
              ...
              <Card key={message.id} className="block border-border/60 bg-card/70 px-4 py-3">
                ...
                <div className="space-y-2">
                  {lines.length > 0 ? lines.map((line, index) => (
                    ...
                    <p className="text-sm text-muted-foreground">{t("chatThread.fallback.noMessageContent")}</p>
                  )}
                </div>
              </Card>
            );
...
function IssueChatChainOfThought({
  ...
  if (isActive) {
    headerVerb = t("chatThread.chainOfThought.working");
    if (liveElapsed) headerSuffix = t("chatThread.chainOfThought.forDuration", { duration: liveElapsed });
  } else if (segmentTiming) {
    const durationMs = segmentTiming.endMs - segmentTiming.startMs;
    const durationText = formatDurationWords(durationMs);
    headerVerb = t("chatThread.chainOfThought.worked");
    if (durationText) headerSuffix = t("chatThread.chainOfThought.forDuration", { duration: durationText });
  } else {
    headerVerb = t("chatThread.chainOfThought.worked");
  }
  ...
  return (
    <div className="group/pre relative">
      ...
      <button
        ...
        title={t("chatThread.toolPart.copy")}
        aria-label={t("chatThread.toolPart.copy")}
        onClick={() => {
          void copyTextToClipboard(children).then(() => {
            ...
            toastActions?.pushToast({
              title: t("chatThread.toolPart.copyFailed"),
              body: error instanceof Error ? error.message : t("chatThread.toolPart.unableToCopy"),
              tone: "error",
            });
          });
        }}
...
function IssueChatUserMessage({
  ...
  const queueBadgeLabel = queueReason === "hold" ? t("chatThread.userMessage.deferredWake") : t("chatThread.userMessage.queued");
    ...
    <div className={cn("flex min-w-0 max-w-(--pct-85) flex-col", isCurrentUser && "items-end")}>
      <div className={cn("mb-1 flex items-center gap-2 px-1", isCurrentUser ? "justify-end" : "justify-start")}>
        ...
        {followUpRequested ? (
          <Badge variant="outline" className="text-(length:--text-nano) uppercase tracking-(--tracking-eyebrow)">
            {t("chatThread.userMessage.followUp")}
          </Badge>
        ) : null}
      </div>
      <div
        ...
        {queued ? (
          <div className="mb-1.5 flex items-center gap-2">
            ...
            {queueTargetRunId && onInterruptQueued ? (
              ...
              >
                {isInterruptingQueuedRun ? t("chatThread.userMessage.interrupting") : t("chatThread.userMessage.interrupt")}
              </Button>
            ) : null}
            {onCancelQueued ? (
              <Button
              ...
              >
                {t("chatThread.userMessage.cancel")}
              </Button>
            ) : null}
          </div>
        ) : null}
        {deleted ? (
          <div className="text-sm italic text-muted-foreground">{t("chatThread.userMessage.commentDeleted")}</div>
        ) : (
      ...
      {pending ? (
        <div className={cn("mt-1 flex px-1 text-(length:--text-micro) text-muted-foreground", isCurrentUser ? "justify-end" : "justify-start")}>
          {t("chatThread.userMessage.sending")}
        </div>
      ) : (
        <div
          className={cn(
        ...
        >
          ...
          {!deleted ? (
            <button
              ...
              title={t("chatThread.userMessage.copyMessage")}
              aria-label={t("chatThread.userMessage.copyMessage")}
              onClick={() => {
                ...
                void copyTextToClipboard(text).then(() => {
                  ...
                  toastActions?.pushToast({
                    title: t("chatThread.userMessage.copyFailed"),
                    body: error instanceof Error ? error.message : t("chatThread.userMessage.unableToCopyMessage"),
                    tone: "error",
                  });
                });
              }}
          ...
          {canDeleteComment ? (
            <button
              ...
              title={t("chatThread.userMessage.deleteComment")}
              aria-label={t("chatThread.userMessage.deleteComment")}
  ...
  return (
    <>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("chatThread.userMessage.deleteCommentTitle")}</DialogTitle>
            <DialogDescription>
              {t("chatThread.userMessage.deleteCommentDescription")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={confirmDeleteComment}>
              {t("chatThread.userMessage.deleteComment")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            title={t("chatThread.assistantMessage.moreActions")}
            aria-label={t("chatThread.assistantMessage.moreActions")}
          >
        ...
        <DropdownMenuContent align="end">
          ...
          >
            ...
            {t("chatThread.userMessage.copyMessage")}
          </DropdownMenuItem>
          ...
          {runHref ? (
            <DropdownMenuItem asChild>
              <Link to={runHref} target="_blank" rel="noreferrer noopener">
                ...
                {t("chatThread.assistantMessage.viewRun")}
              </Link>
            </DropdownMenuItem>
          ) : null}
        </DropdownMenuContent>
    ...
    return (
      <div id={anchorId}>
        <div className="group flex flex-col items-start py-1.5">
          ...
          >
            {deleted ? (
              <div className="text-sm italic text-muted-foreground">{t("chatThread.assistantMessage.deleted")}</div>
            ) : (
              <div className="min-w-0 max-w-full space-y-3">
        ...
        <div className="min-w-0 flex-1">
            ...
            <div className="mb-1.5 flex items-center gap-2">
              ...
              {isRunning ? (
                ...
                >
                  ...
                  {t("chatThread.assistantMessage.running")}
                </Badge>
              ) : null}
            </div>
          )}
          ...
          {deleted ? (
            <div className="rounded-sm bg-muted/40 px-3 py-2 text-sm italic text-muted-foreground">
              {t("chatThread.assistantMessage.deleted")}
            </div>
          ) : !folded ? (
            <>
  ...
  return (
    <>
      <button
        ...
        title={t("chatThread.feedback.helpful")}
        aria-label={t("chatThread.feedback.helpful")}
      ...
      <Popover open={reasonOpen} onOpenChange={setReasonOpen}>
        <PopoverTrigger asChild>
          <button
            ...
            title={t("chatThread.feedback.needsWork")}
            aria-label={t("chatThread.feedback.needsWork")}
        ...
        <PopoverContent side="top" align="start" className="w-80 p-3">
          <div className="mb-2 text-sm font-medium">{t("chatThread.feedback.whatCouldHaveBeenBetter")}</div>
          <Textarea
            value={downvoteReason}
            onChange={(event) => setDownvoteReason(event.target.value)}
            placeholder={t("chatThread.feedback.addShortNote")}
          ...
          <div className="mt-2 flex items-center justify-end gap-2">
            ...
            >
              {isSaving ? t("chatThread.feedback.saving") : t("chatThread.feedback.saveNote")}
            </Button>
          </div>
        </PopoverContent>
      </Popover>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("chatThread.feedback.savePreferenceTitle")}</DialogTitle>
            <DialogDescription>
              {t("chatThread.feedback.savePreferenceDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>{t("chatThread.feedback.savedLocally")}</p>
            <p>
              <Trans
                i18nKey="chatThread.feedback.alwaysAllowSentence"
                components={{ 1: <span className="font-medium text-foreground" /> }}
              />
              {" "}
              <Trans
                i18nKey="chatThread.feedback.dontAllowSentence"
                components={{ 1: <span className="font-medium text-foreground" /> }}
              />
            </p>
            <p>{t("chatThread.feedback.changeInSettings")}</p>
            {termsUrl ? (
              <a
                href={termsUrl}
                target="_blank"
              ...
              >
                {t("chatThread.feedback.readTerms")}
              </a>
            ) : null}
          </div>
          <DialogFooter>
            ...
            >
              {isSaving ? t("chatThread.feedback.saving") : t("chatThread.feedback.dontAllow")}
            </Button>
            <Button
              type="button"
              disabled={!pendingSharingDialog || isSaving}
            ...
            >
              {isSaving ? t("chatThread.feedback.saving") : t("chatThread.feedback.alwaysAllow")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
  ...
  const rowContent = (
    <div className="min-w-0 flex-1">
      <div className={cn("flex flex-wrap items-center gap-x-1.5 gap-y-1 text-xs", isCurrentUser && "justify-end")}>
        ...
        <span className="text-muted-foreground">{t("chatThread.systemNotice.updatedThisTask")}</span>
        <a
          href={anchorId ? `#${anchorId}` : undefined}
        ...
        >
          ...
          {expanded ? t("chatThread.activity.hideConfirmation") : t("chatThread.activity.expiredConfirmation")}
        </button>
      </div>
      {expanded ? (
...
function StaleDispositionWarningDetails({
  ...
  if (sections.length === 0) {
    return <div className="text-xs leading-5 text-muted-foreground">{t("chatThread.systemNotice.noAdditionalDetails")}</div>;
  }
        ...
        <div className="min-w-0 flex-1">
          ...
          >
            <span className="text-sm font-medium text-foreground/80">
              {t("chatThread.systemNotice.staleDispositionWarning")}
            </span>
            <span className="ml-auto flex items-center gap-1.5">
              {message.createdAt ? (
...
function SystemNoticeCommentRow({
  ...
  const handleCopy = () => {
    void copyTextToClipboard(bodyText).then(() => {
      ...
      toastActions?.pushToast({
        title: t("chatThread.userMessage.copyFailed"),
        body: error instanceof Error ? error.message : t("chatThread.systemNotice.unableToCopySystemNotice"),
        tone: "error",
      });
    });
  };
  ...
  const handleCopyLink = () => {
    ...
    void copyTextToClipboard(url).then(() => {
      ...
      toastActions?.pushToast({
        title: t("chatThread.userMessage.copyFailed"),
        body: error instanceof Error ? error.message : t("chatThread.systemNotice.unableToCopySystemNoticeLink"),
        tone: "error",
      });
    });
  };
        ...
        <div className="mt-1 flex items-center justify-end gap-1.5 px-1 opacity-0 transition-opacity group-hover:opacity-100">
          ...
          {anchorId ? (
            <button
              type="button"
              className="inline-flex h-6 w-6 items-center justify-center text-muted-foreground transition-colors hover:text-foreground"
              title={t("chatThread.systemNotice.copyLink")}
              aria-label={t("chatThread.systemNotice.copyLinkToSystemNotice")}
          ...
          <button
            type="button"
            className="inline-flex h-6 w-6 items-center justify-center text-muted-foreground transition-colors hover:text-foreground"
            title={t("chatThread.systemNotice.copyNoticeText")}
            aria-label={t("chatThread.systemNotice.copySystemNotice")}
            onClick={handleCopy}
          >
            {copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
          </button>
      ...
      <IssueChatMetadataRow anchorId={anchorId} icon={rowIcon}>
        <div className="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5 text-xs">
          ...
          <span className="text-muted-foreground">
            {custom.followUpRequested === true ? t("chatThread.systemNotice.requestedFollowUp") : t("chatThread.systemNotice.updatedThisTask")}
          </span>
          <a
            href={anchorId ? `#${anchorId}` : undefined}
    ...
    return (
      <IssueChatMetadataRow anchorId={anchorId} icon={rowIcon}>
        <div className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-xs">
          ...
          <span className="text-muted-foreground">{t("chatThread.systemNotice.runNoun")}</span>
          <Link
  ...
  return (
    <div id={anchorId} className="flex items-start gap-2.5 py-1.5">
      ...
      <div className="min-w-0 rounded-md border border-dashed border-border bg-muted/20 px-3 py-2 text-sm text-muted-foreground">
        ...
        <span> {t("chatThread.deletedComment.deletedThisComment")}</span>
  ...
  async function attachFile(file: File) {
    ...
    try {
      if (onImageUpload && file.type.startsWith("image/")) {
        ...
        setComposerAttachments((prev) => prev.map((item) =>
          item.id === attachmentId
            ? { ...item, status: "error", error: t("chatThread.composer.unableToAttachType") }
            : item,
        ));
      }
    } catch (err) {
      setComposerAttachments((prev) => prev.map((item) =>
        item.id === attachmentId
          ? {
              ...item,
              status: "error",
              error: err instanceof Error ? err.message : t("chatThread.composer.uploadFailed"),
            }
          : item,
      ));
    }
  ...
  return (
    ...
    >
        ...
        >
          <div className="flex max-w-md items-center gap-3 rounded-md bg-background/80 px-3 py-2 text-left shadow-sm ring-1 ring-border/60">
            ...
            <div className="min-w-0">
              <div className="text-sm font-medium text-foreground">{t("chatThread.composer.dropToUpload")}</div>
              <div className="mt-0.5 text-xs leading-5 text-muted-foreground">
                {t("chatThread.composer.dropHint")}
              </div>
            </div>
          </div>
        </div>
      ...
      <MarkdownEditor
        ...
        value={body}
        onChange={setBody}
        placeholder={t("chatThread.composer.replyPlaceholder")}
        mentions={mentions}
          ...
          {composerAttachments.map((attachment) => {
            ...
            const statusLabel =
              attachment.status === "uploading"
                ? t("chatThread.composer.uploadingToTask")
                : attachment.status === "error"
                  ? attachment.error ?? t("chatThread.composer.uploadFailed")
                  : attachment.inline
                    ? t("chatThread.composer.insertedInline")
                    : t("chatThread.composer.attachedToTask");
            return (
              <div
                key={attachment.id}
                className={cn(
      ...
      <div className="flex flex-wrap items-center justify-end gap-3">
          ...
          {(onImageUpload || onAttachImage) ? (
            <>
              ...
              <Button
                ...
                disabled={attaching}
                title={t("chatThread.composer.attachFile")}
              >
                <Paperclip className="h-4 w-4" />
              </Button>
            </>
            ...
            <Popover open={workModeMenuOpen} onOpenChange={setWorkModeMenuOpen}>
              ...
              >
                ...
                <div className="mt-1 border-t px-2 py-1.5 text-(length:--text-nano) text-muted-foreground">
                  {t("chatThread.composer.cyclesModes")}
                </div>
              </PopoverContent>
            </Popover>
          ) : null}
        ...
        {enableReassign && reassignOptions.length > 0 ? (
          <InlineEntitySelector
            ...
            placeholder={t("chatThread.composer.responsible")}
            noneLabel={t("chatThread.composer.noResponsible")}
            searchPlaceholder={t("chatThread.composer.searchResponsible")}
            emptyMessage={t("chatThread.composer.noResponsibleFound")}
            onChange={setReassignTarget}
            className="h-8 text-xs"
            renderTriggerValue={(option) => {
              if (!option) return <span className="text-muted-foreground">{t("chatThread.composer.responsible")}</span>;
        ...
        <Button size="sm" disabled={!canSubmit} onClick={() => void handleSubmit()}>
          {submitting ? t("chatThread.composer.posting") : t("chatThread.composer.send")}
        </Button>
      </div>
      ...
      <AlertDialog open={noAssigneeDialogOpen} onOpenChange={setNoAssigneeDialogOpen}>
        ...
        >
          <AlertDialogHeader>
            <AlertDialogTitle>{t("chatThread.composer.noResponsibleTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("chatThread.composer.noResponsibleDescription")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
            ...
            >
              {t("chatThread.composer.sendAnyway")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
...
export function IssueChatThread({
  ...
  const resolvedEmptyMessage = emptyMessage
    ?? (variant === "embedded"
      ? t("chatThread.thread.emptyEmbedded")
      : t("chatThread.thread.emptyFull"));
      ...
      <div className={cn(variant === "embedded" ? "space-y-3" : "space-y-4")}>
        {resolvedShowJumpToLatest ? (
          <div className="flex justify-end">
            ...
            >
              {t("chatThread.thread.jumpToLatest")}
            </button>
          </div>
        ) : null}
              ...
              {showComposer ? (
                <div data-testid="issue-chat-thread-notices" className="space-y-2">
                  ...
                  {legacyRecoverySourceIssue ? (
                    <SystemNotice
                      tone="info"
                      label={t("chatThread.systemNotice.legacyRecoveryTask")}
                      body={
                        <span>
                          {t("chatThread.systemNotice.legacyRecoveryTaskBody")}
