  ...
  return (
    ...
    >
      ...
      <div className="space-y-1">
        <div className="text-base font-semibold">{t("search.zeroResults.heading")}</div>
        <p className="text-sm text-muted-foreground">
          {t("search.zeroResults.matchCount", { count: unfilteredTotal })}
          {query ? <> &ldquo;{query}&rdquo;</> : null}
          {t("search.zeroResults.butFiltersHide", { count: activeCount })}
        </p>
      </div>
      ...
      {suggestions.length > 0 ? (
        <div className="flex w-full flex-col gap-1.5">
          <div className="text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
            {t("search.zeroResults.loosenAFilter")}
          </div>
          {suggestions.map((suggestion) => (
            ...
            >
              ...
              <span className="shrink-0 tabular-nums text-emerald-600 dark:text-emerald-400">
                {t("search.zeroResults.additionalCount", { count: suggestion.additionalCount })}
              </span>
            </button>
          ))}
        </div>
      ...
      <Button onClick={onClearAll} variant="default" size="sm">
        ...
        {t("search.zeroResults.clearAllFilters")}
      </Button>
    </div>
  );
}
