  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={buttonVariant} size={iconOnly ? "icon" : "sm"} className={`text-xs ${iconOnly ? "relative h-8 w-8 shrink-0" : ""} ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`} title={iconOnly ? (activeFilterCount > 0 ? t("common.filter") : t("common.filter")) : undefined}>
          <Filter className={iconOnly ? "h-3.5 w-3.5" : "h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"} />
          {!iconOnly && <span className="hidden sm:inline">{t("common.filter")}</span>}
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div className="min-w-0 space-y-3">
              ...
              {creatorOptions.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("common.creator")}</span>
                      ...
                      {selectedCreatorOptions.map((creator) => (
                        <Badge key={creator.id} variant="secondary" className="gap-1 pr-1">
                          ...
                          <button
                            ...
                            aria-label={t("aria.removeCreator", { name: creator.label })}
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </Badge>
              ...
              <div className="space-y-1">
                ...
                {enableLintResidualTaskFilter ? (
                  <label
                    ...
                    title={t("filters.hideLintResidualTasksTitle", { prefix: LINT_RESIDUAL_TASK_TITLE_PREFIX })}
                  >
                    <Checkbox
                      checked={state.hideLintResidualTasks}
                      onCheckedChange={(checked) => onChange({ hideLintResidualTasks: checked === true })}
                    />
                    <span className="text-sm">{t("filters.hideLintResidualTasks")}</span>
                  </label>
                ) : null}
                {enableHourlyLogRotationTaskFilter ? (
                  <label
                    className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50"
                    title={t("filters.hideHourlyLogRotationTasksTitle", { prefix: HOURLY_LOG_ROTATION_TASK_TITLE_PREFIX })}
                  >
                    <Checkbox
                      checked={state.hideHourlyLogRotationTasks}
                      onCheckedChange={(checked) => onChange({ hideHourlyLogRotationTasks: checked === true })}
                    />
                    <span className="text-sm">{t("filters.hideHourlyLogRotationTasks")}</span>
                  </label>
                ) : null}
                {enablePrefixedTaskFilter ? (
                  <label
                    className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50"
                    title={t("filters.hidePrefixedTasksTitle", { prefix1: "Paperclip:", prefix2: "Lint:" })}
                  >
                    <Checkbox
                      checked={state.hidePrefixedTasks ?? false}
                      onCheckedChange={(checked) => onChange({ hidePrefixedTasks: checked === true })}
                    />
                    <span className="text-sm">{t("filters.hidePrefixedTasks")}</span>
                  </label>
                ) : null}
                {enableProductivityReviewFilter ? (
                  <label
                    className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50"
                    title={t("filters.hideProductivityReviewIssuesTitle")}
                  >
                    <Checkbox
                    ...
                    />
                    <span className="text-sm">{t("filters.hideProductivityReviewIssues")}</span>
                  </label>
                ) : null}
              </div>
            </div>
