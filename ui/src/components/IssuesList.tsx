...
function SubIssueProgressSummaryStrip({
  ...
  const issueStatusLabels: Record<IssueStatus, string> = {
    ...
    cancelled: t("issues.status.cancelled"),
  };
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
            <span className="font-medium text-foreground">
              {t("issuesList.progress.doneOf", { done: summary.doneCount, total: summary.totalCount })}
            </span>
            <span className="text-muted-foreground">
              {t("issuesList.progress.inProgress", { count: summary.inProgressCount })}
            </span>
            <span className="text-muted-foreground">
              {t("issuesList.progress.blocked", { count: summary.blockedCount })}
            </span>
            {showCostSummary && (
              <>
                <span
                ...
                >
                  {t("issuesList.progress.tokens", { value: formatTokens(totalTokens) })}
                </span>
                <span className="text-muted-foreground tabular-nums">
                  {t("issuesList.costSummary.runtime", { value: formatDurationMs(costSummary.runtimeMs) })}
                </span>
              </>
            )}
          </div>
          <div
            role="progressbar"
            aria-label={t("issuesList.progress.ariaLabel")}
            aria-valuemin={0}
          ...
          {target && targetIssue ? (
            <>
              <div className="text-xs font-medium text-muted-foreground">
                {target.kind === "next" ? t("issuesList.progress.nextUp") : t("issuesList.progress.waitingOnBlockers")}
              </div>
              <Link
            ...
            <div className="text-sm font-medium text-foreground">{t("issuesList.progress.empty.noActive")}</div>
          ) : summary.doneCount === summary.totalCount ? (
            <div className="text-sm font-medium text-foreground">{t("issuesList.progress.empty.allDone")}</div>
          ) : (
            <div className="text-sm font-medium text-foreground">{t("issuesList.progress.empty.noActionable")}</div>
          )}
        </div>
      </div>
    </div>
...
export function IssuesList({
  ...
  const creatorOptions = useMemo<CreatorOption[]>(() => {
    ...
    if (currentUserId) {
      options.set(`user:${currentUserId}`, {
        ...
        label: currentUserId === "local-board" ? t("issuesList.creator.board") : t("issuesList.creator.me"),
        kind: "user",
  ...
  const createActionLabel = createIssueLabel
    ? t("issuesList.action.createWithKind", { kind: createIssueLabel })
    : t("issuesList.action.createIssue");
  const createButtonLabel = createIssueLabel
    ? t("issuesList.action.newWithKind", { kind: createIssueLabel })
    : t("issuesList.action.newIssue");
  ...
  return (
    <div ref={rootRef} className="space-y-4">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          {viewState.viewMode === "list" && (
            <Button
              ...
              title={viewState.nestingEnabled ? t("inbox.nesting.disable") : t("inbox.nesting.enable")}
            >
          ...
          {viewState.viewMode === "board" && (
            <>
              <Button
                ...
                title={boardCompactCards ? t("issuesList.board.comfortable") : t("issuesList.board.compact")}
              >
              ...
              <Button
                ...
                title={boardCollapsedStatuses.length > 0 ? t("issuesList.board.expandCold") : t("issuesList.board.collapseCold")}
              >
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    title={t("issuesList.board.cardsPerColumn")}
                  >
                ...
                <PopoverContent align="end" className="w-40 p-0">
                  <div className="p-2 space-y-0.5">
                    {KANBAN_COLUMN_PAGE_SIZE_OPTIONS.map((pageSize) => (
                      ...
                      >
                        <span>{t("issuesList.board.perColumn", { count: pageSize })}</span>
              ...
              <Button
                ...
                title={t("issuesList.board.resetDensity")}
              >
          ...
          <IssueColumnPicker
            ...
            title={t("issuesList.columnPicker.title")}
            iconOnly
          />
          ...
          <IssueFiltersPopover
            ...
            enableHourlyLogRotationTaskFilter={enableHourlyLogRotationTaskFilter}
            enablePrefixedTaskFilter={enablePrefixedTaskFilter}
            iconOnly
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("issuesList.toolbar.sort")}>
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("issuesList.toolbar.group")}>
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ...
                    ["project", t("issuesList.group.project")],
                    ["workspace", t("issuesList.group.workspace")],
                    ["parent", t("issuesList.group.parent")],
      ...
      {!searchWithinLoadedIssues && normalizedIssueSearch.length > 0 && searchedIssues.length === ISSUE_SEARCH_RESULT_LIMIT && filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">
          {t("issuesList.search.limitReached", { limit: ISSUE_SEARCH_RESULT_LIMIT })}
        </p>
      )}
      {boardColumnLimitReached && (
        <p className="text-xs text-muted-foreground">
          {t("issuesList.board.columnLimitReached", { limit: ISSUE_BOARD_COLUMN_RESULT_LIMIT })}
        </p>
      )}
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("issuesList.empty.noMatch")}
        ...
        <>
              ...
              <IssueGroupHeader
                ...
                trailing={(
                  <Button
                    ...
                    title={t("inbox.action.newIssueIn", { group: group.label ?? "" })}
                    aria-label={t("inbox.action.newIssueIn", { group: group.label ?? "" })}
                ...
                const renderIssueRow = (issue: Issue, depth: number) => {
                  ...
                  const visibleBlockerChips = unresolvedVisibleBlockers
                    .map((blockerId) => {
                      ...
                      const blockerStepSuffix = blockerStep ? t("issuesList.blocker.stepSuffix", { step: blockerStep }) : "";
                      return { blockerId, chipLabel: t("issuesList.blocker.blockedBy", { label, stepSuffix: blockerStepSuffix }) };
                    })
                  ...
                  const additionalVisibleBlockerLabel = additionalVisibleBlockerCount > 0
                    ? t("issuesList.blocker.andMore", { count: additionalVisibleBlockerCount })
                    : "";
                          ...
                          <>
                            {hasChildren && !isExpanded ? (
                              <span className="ml-1.5 text-xs text-muted-foreground">
                                ({t("issuesList.tree.subTaskCount", { count: totalDescendants })})
                              </span>
                            ) : null}
                            {issueBadge ? (
                              issueBadge === "Paused" ? (
                                <span
                                  ...
                                  aria-label={t("issuesList.badge.paused")}
                                  title={t("issuesList.badge.paused")}
                                >
                                  <CircleSlash2 className="h-3 w-3" />
                                  {t("issuesList.badge.paused")}
                                </span>
                              ) : (
                            ...
                            {isSuccessfulRunHandoffRequired(issue) ? (
                              <span
                                ...
                                aria-label={t("issuesList.badge.needsNextStep")}
                                title={t("issuesList.badge.needsNextStep")}
                              >
                                <CircleDot className="h-3 w-3" />
                                {t("issuesList.badge.needsNextStep")}
                              </span>
                            ) : null}
                          </>
                        )}
                                    ...
                                    >
                                      {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                                        ...
                                        <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
                                          ...
                                          {t("issuesList.assignee.unassigned")}
                                        </span>
                                      )}
                                    </button>
                                  </PopoverTrigger>
          ...
          {(remainingIssueRowCount > 0 || hasMoreIssues || isLoadingMoreIssues) && (
            <div className="py-2" data-testid="issues-load-more-sentinel">
              <p className="text-xs text-muted-foreground">
                {isLoadingMoreIssues
                  ? t("issuesList.loadMore.loading")
                  : remainingIssueRowCount > 0
                    ? t("issuesList.loadMore.rendering", {
                        rendered: Math.min(renderedIssueRowLimit, filtered.length),
                        total: filtered.length,
                      })
                    : t("issuesList.loadMore.scroll")}
              </p>
            </div>
          )}
        </>
