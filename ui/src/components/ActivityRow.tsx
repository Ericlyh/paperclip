...
export function ActivityRow({ event, agentMap, userProfileMap, entityNameMap, entityTitleMap, className }: ActivityRowProps) {
  ...
  const actorName = actor?.name ?? (event.actorType === "system" ? t("common.system") : userProfile?.label ?? (event.actorType === "user" ? t("common.board") : event.actorId || t("activity.unknownActor")));
