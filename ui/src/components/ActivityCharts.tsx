...
export function RunActivityChart(props: RunChartProps) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("charts.noRuns")}</p>;

  const legendItems = [
    { color: runSegmentColors.succeeded, label: t("status.succeeded") },
    ...(hasRecovered ? [{ color: runSegmentColors.recovered, label: t("status.recovered") }] : []),
    { color: runSegmentColors.failed, label: t("status.failed") },
    { color: runSegmentColors.other, label: t("status.other") },
  ];
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "var(--hex-6b7280)", label: statusLabels[s] ? t(statusLabels[s]) : s }))} />
    </div>
  );
}
