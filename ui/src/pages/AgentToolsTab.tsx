  ...
  return (
    <section className="rounded-lg border border-border bg-card">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border px-3 py-2.5">
        <div>
          <h3 className="text-sm font-semibold text-foreground">{t("agentToolsTab.installedApps.title")}</h3>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {t("agentToolsTab.installedApps.subtitle", { agentName })}
          </p>
        </div>
                ...
                <div key={connection.id} className="space-y-2 px-3 py-3">
                  <label className="flex items-start gap-3">
                    <Checkbox
                      ...
                      aria-label={t("agentToolsTab.installedApps.installAria", { name: connection.name, agentName })}
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="flex flex-wrap items-center gap-2">
                        ...
                        {rowPending ? <span className="text-xs text-muted-foreground">{t("agentToolsTab.status.saving")}</span> : null}
                      </span>
                  ...
                  {permitted && !checked ? (
                    <InlineBanner
                      ...
                      actions={(
                        ...
                        >
                          {t("agentToolsTab.installedApps.openPermissions")}
                        </Link>
                      )}
                    >
                      {t("agentToolsTab.installedApps.permittedButNotInstalled")}
                    </InlineBanner>
                  ) : null}
                </div>
              );
...
function InstallBadge({
  ...
  const label = installed
    ? (installedForAll ? t("agentToolsTab.installedApps.installedForAll") : t("agentToolsTab.installedApps.installed"))
    : permitted
      ? t("agentToolsTab.installedApps.permittedOnly")
      : t("agentToolsTab.installedApps.notPermitted");
  return (
    <span
      className={cn(
...
function InstallSaveStatusChip({
  ...
  const { t } = useTranslation();
  if (pending) return <span className="text-xs text-muted-foreground">{t("agentToolsTab.status.saving")}</span>;
  if (error) return <span className="text-xs text-destructive">{t("agentToolsTab.status.saveError")}</span>;
  if (unsaved) return <span className="text-xs text-muted-foreground">{t("agentToolsTab.status.unsaved")}</span>;
...
export function AgentToolsTab({ agent, companyId }: { agent: AgentDetailRecord; companyId: string }) {
  ...
  if (effective.isLoading) return <ToolsLoadingState label={t("agentToolsTab.loading")} />;
  if (effective.error) {
  ...
  return (
    <div className="space-y-4">
      <EnforcementBanner
        tone="info"
        title={t("agentToolsTab.enforcement.title")}
        body={
          <>
            {t("agentToolsTab.enforcement.bodyBefore", { agentName: agent.name })}{" "}
            <span className="font-medium">{agent.name}</span>
            {t("agentToolsTab.enforcement.bodyBetween")}{" "}
            <span className="font-medium">{t("agentToolsTab.enforcement.cannotExpand")}</span>
            {t("agentToolsTab.enforcement.bodyAfter")}
          </>
        }
      />
        ...
        <div className="lg:col-span-2">
          <div className="rounded-lg border border-border">
            <div className="flex items-center justify-between gap-2 border-b border-border px-3 py-2.5">
              <h3 className="text-sm font-semibold text-foreground">{t("agentToolsTab.allowedTools.title")}</h3>
              <span className="text-xs text-muted-foreground tabular-nums">
                {allowedTools.length} {allowedTools.length === 1 ? t("agentToolsTab.allowedTools.tool_one") : t("agentToolsTab.allowedTools.tool_other")}
              </span>
            </div>
            {allowedTools.length === 0 ? (
              <p className="px-3 py-6 text-sm text-muted-foreground">
                {t("agentToolsTab.allowedTools.empty")}
              </p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left text-xs text-muted-foreground">
                    <th className="px-3 py-2 font-medium">{t("agentToolsTab.allowedTools.colTool")}</th>
                    <th className="px-3 py-2 font-medium">{t("agentToolsTab.allowedTools.colCapability")}</th>
                    <th className="px-3 py-2 font-medium">{t("agentToolsTab.allowedTools.colRisk")}</th>
                    <th className="px-3 py-2 font-medium">{t("agentToolsTab.allowedTools.colSource")}</th>
                  </tr>
                </thead>
        ...
        <aside className="space-y-4">
          <div className="rounded-lg border border-border bg-background/60 p-3">
            <h3 className="flex items-center gap-1.5 text-sm font-semibold text-foreground">
              ...
              {t("agentToolsTab.sidePanel.whyTheseTools")}
            </h3>
            ...
            <div className="mt-3 space-y-1.5">
              <div className="flex items-center justify-between gap-2">
                <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
                  {t("agentToolsTab.sidePanel.accessProfiles")}
                </div>
                <Link
                  to={`${profilesHref}?check=1`}
                  className="text-(length:--text-micro) font-medium text-primary hover:underline"
                >
                  {t("agentToolsTab.sidePanel.checkAccess")}
                </Link>
              </div>
              {profiles.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  {t("agentToolsTab.sidePanel.noProfile")}
                </p>
              ) : (
                      ...
                      {profile.summary.isCompanyDefault ? (
                        <div className="mt-1">
                          <span className="rounded border border-border px-1.5 py-0.5 text-(length:--text-nano) uppercase text-muted-foreground">
                            {t("agentToolsTab.sidePanel.companyDefault")}
                          </span>
                        </div>
                      ) : null}
                    </div>
            ...
            <div className="mt-3 space-y-1.5">
              <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
                {t("agentToolsTab.sidePanel.activePolicies")}
              </div>
              {policiesQuery.isLoading ? (
                <p className="text-xs text-muted-foreground">{t("agentToolsTab.sidePanel.loadingPolicies")}</p>
              ) : governingPolicies.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  {t("agentToolsTab.sidePanel.noPolicy")}
                </p>
              ) : (
                  ...
                  <div key={policy.id} className="rounded-md border border-border/70 px-2.5 py-2">
                    <div className="flex items-center justify-between gap-2">
                      <Link
                        ...
                        title={t("agentToolsTab.sidePanel.policyTitle", { order, name: policy.name })}
                      >
                        #{order} {policy.name}
                      </Link>
            ...
            <div className="mt-3 space-y-1.5">
              <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
                {t("agentToolsTab.sidePanel.unavailableTools")}
              </div>
              {catalogLoading ? (
                <p className="text-xs text-muted-foreground">{t("agentToolsTab.sidePanel.checkingTools")}</p>
              ) : deniedTools.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  {t("agentToolsTab.sidePanel.allAllowed")}
                </p>
              ) : (
                <>
                  <p className="text-(length:--text-micro) text-muted-foreground">
                    {t("agentToolsTab.sidePanel.deniedToolsNote")}
                  </p>
                  ...
                  {deniedTools.length > DENIED_TOOLS_DISPLAY_LIMIT ? (
                    <p className="text-(length:--text-micro) text-muted-foreground">
                      {t("agentToolsTab.sidePanel.more", { count: deniedTools.length - DENIED_TOOLS_DISPLAY_LIMIT })}
                    </p>
                  ) : null}
                </>
              )}
