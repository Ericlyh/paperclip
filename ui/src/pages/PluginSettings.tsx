...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("pluginPage.plugins"), href: "/company/settings/instance/plugins" },
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">{t("pluginPage.loading", { defaultValue: "Loading plugin details..." })}</div>;
  }
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || t("pluginPage.noDescription", { defaultValue: "No description provided." });
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "configuration" | "status")} className="space-y-6">
        <PageTabBar
          align="start"
          items={[
            { value: "configuration", label: t("pluginSettings.tabs.configuration") },
            { value: "status", label: t("pluginSettings.tabs.status") },
          ]}
          value={activeTab}
            ...
            <section className="space-y-4">
              ...
              {hasCustomSettingsPage ? (
                ...
                <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm">
                  <p className="font-medium text-foreground">{t("pluginSettings.configureInEnvironments")}</p>
                ...
                <p className="text-sm text-muted-foreground">
                  {t("pluginSettings.noSettingsRequired")}
                </p>
              ) : null}
            </section>
          </div>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {t("pluginSettings.runtimeDashboardDescription")}
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div>
                        ...
                        {dashboardData.worker ? (
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  ...
                                  <span className="text-xs">
                                    {t("pluginSettings.crashesBreakdown", {
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("pluginSettings.noWorkerRegistered")}</p>
                        )}
                      </div>
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{t("pluginSettings.lastLogEntries", { count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
                ...
                <CardContent>
                  {healthLoading ? (
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      ...
                      <p>{t("pluginSettings.healthChecksWhenReady")}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
  ...
  if (!companyId) {
    return (
      <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
        {t("pluginLocalFolders.selectCompany")}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-medium">{t("pluginLocalFolders.heading")}</h3>
      </div>
      {error ? (
        <div className="rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          {(error as Error).message || t("pluginLocalFolders.loadFailed")}
        </div>
      ) : null}
      {isLoading ? (
        <div className="flex items-center gap-2 py-3 text-sm text-muted-foreground">
          <Loader2 className="h-4 w-4 animate-spin" />
          {t("pluginLocalFolders.loading")}
        </div>
      ) : (
        <div className="space-y-3">
...
function PluginLocalFolderRow({ pluginId, companyId, declaration, status }: PluginLocalFolderRowProps) {
    ...
    onSuccess: (nextStatus) => {
      setMessage({
        ...
        text: nextStatus.healthy
          ? t("pluginLocalFolder.saved")
          : t("pluginLocalFolder.savedWithValidationWarning"),
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.plugins.localFolders(pluginId, companyId) });
    },
    onError: (err: Error) => {
      setMessage({ type: "error", text: err.message || t("pluginLocalFolder.saveFailed") });
    },
  });
  ...
  const handleSave = useCallback(() => {
    if (!trimmedPath) {
      setMessage({ type: "error", text: t("pluginLocalFolder.pathRequired") });
      return;
    }
    if (!isLikelyAbsolutePath(trimmedPath)) {
      setMessage({ type: "error", text: t("pluginLocalFolder.pathMustBeAbsolute") });
      return;
    }
    setMessage(null);
    ...
    <div className="space-y-4 rounded-md border border-border/70 bg-background px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={status?.healthy ? "default" : "secondary"}>
              {status?.healthy ? t("pluginLocalFolder.healthy") : t("pluginLocalFolder.needsAttention")}
            </Badge>
          </div>
        ...
        <Badge variant={access === "readWrite" ? "default" : "outline"}>
          {access === "readWrite" ? t("pluginLocalFolder.readWrite") : t("pluginLocalFolder.readOnly")}
        </Badge>
      </div>

      <div className="grid gap-3 text-sm sm:grid-cols-3">
        <FolderStatusMetric label={t("pluginLocalFolder.configured")} value={status?.configured ? t("common.yes") : t("common.no")} ok={!!status?.configured} />
        <FolderStatusMetric label={t("pluginLocalFolder.readable")} value={status?.readable ? t("common.yes") : t("common.no")} ok={!!status?.readable} />
        <FolderStatusMetric
          label={t("pluginLocalFolder.writable")}
          value={access === "read" ? t("pluginLocalFolder.notRequested") : status?.writable ? t("common.yes") : t("common.no")}
      ...
      {status?.path ? (
        <div className="space-y-1 text-sm">
          <div className="text-xs font-medium text-muted-foreground">{t("pluginLocalFolder.configuredPath")}</div>
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-muted-foreground" htmlFor={`local-folder-${declaration.folderKey}`}>
          {t("pluginLocalFolder.localFolderPath")}
        </label>
        <div className="flex items-center gap-2">
          <input
      ...
      {status?.problems?.length ? (
        <div className="space-y-2 rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          <div className="font-medium">{t("pluginLocalFolder.validationProblems")}</div>
...
function FolderRequirements({
  ...
  return (
    <div className="grid gap-3 text-sm md:grid-cols-2">
      <RequirementList
        title={t("pluginLocalFolder.requiredDirectories")}
        items={requiredDirectories}
        missingItems={missingDirectories}
        missingLabel={t("pluginLocalFolder.missingDirectories")}
        inspectionUnavailable={rootNotInspected}
      />
      <RequirementList
        title={t("pluginLocalFolder.requiredFiles")}
        items={requiredFiles}
        missingItems={missingFiles}
        missingLabel={t("pluginLocalFolder.missingFiles")}
        inspectionUnavailable={rootNotInspected}
      />
    </div>
  );
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {inspectionUnavailable ? (
          <Badge variant="secondary" className="text-(length:--text-nano)">
            {t("pluginLocalFolder.notInspected")}
          </Badge>
        ) : missingItems.length > 0 ? (
          <Badge variant="destructive" className="text-(length:--text-nano)">
            {t("pluginLocalFolder.missingCount", { count: missingItems.length })}
          </Badge>
        ) : (
          <Badge variant="outline" className="text-(length:--text-nano)">{t("pluginLocalFolder.present")}</Badge>
        )}
      </div>
      {items.length > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">{t("pluginLocalFolder.noneDeclared")}</p>
      )}
      {inspectionUnavailable ? (
        <p className="text-xs text-amber-700 dark:text-amber-300">{t("pluginLocalFolder.configuredRootNotInspected")}</p>
      ) : missingItems.length > 0 ? (
        <p className="text-xs text-destructive">{t("pluginLocalFolder.missingItemsList", { label: missingLabel, items: missingItems.join(", ") })}</p>
      ) : null}
    </div>
  );
}
...
function PluginConfigForm({ pluginId, companyId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: (configJson: Record<string, unknown>) => {
      if (!companyId) throw new Error(t("pluginConfigForm.selectCompanyBeforeSave"));
      return pluginsApi.saveConfig(pluginId, companyId, configJson);
    },
    onSuccess: () => {
      setSaveMessage({ type: "success", text: t("pluginConfigForm.saved") });
      setTestResult(null);
      if (companyId) {
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("pluginConfigForm.saveFailed") });
    },
  });
  ...
  const testMutation = useMutation({
    mutationFn: (configJson: Record<string, unknown>) => {
      if (!companyId) throw new Error(t("pluginConfigForm.selectCompanyBeforeTest"));
      return pluginsApi.testConfig(pluginId, companyId, configJson);
    },
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: t("pluginConfigForm.testPassed") });
      } else {
        setTestResult({ type: "error", text: result.message || t("pluginConfigForm.testFailed") });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t("pluginConfigForm.testFailed") });
    },
  });
      ...
      <div className="flex items-center gap-2 pt-2">
        ...
        >
          {saveMutation.isPending ? (
            <>
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
              {t("pluginConfigForm.saving")}
            </>
          ) : (
            t("pluginConfigForm.save")
          )}
        </Button>
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              <>
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {t("pluginConfigForm.testing")}
              </>
            ) : (
              t("pluginConfigForm.test")
            )}
          </Button>
        )}
      </div>
...
function formatDuration(ms: number, t: (key: string, options?: Record<string, unknown>) => string): string {
  if (ms < 1000) return t("time.secondsCompact", { value: ms, defaultValue: `${ms}ms` });
  if (ms < 60000) return t("time.secondsCompact", { value: (ms / 1000).toFixed(1), defaultValue: `${(ms / 1000).toFixed(1)}s` });
  return t("time.minutesCompact", { value: (ms / 60000).toFixed(1), defaultValue: `${(ms / 60000).toFixed(1)}m` });
}
...
function formatRelativeTime(isoString: string, t: (key: string, options?: Record<string, unknown>) => string): string {
  ...
  if (diffMs < 0) return t("time.justNow", { defaultValue: "just now" });
  const seconds = Math.floor(diffMs / 1000);
  if (seconds < 60) return t("time.secondsCompact", { value: seconds, defaultValue: `${seconds}s ago` });
  const minutes = Math.floor(seconds / 60);
  if (minutes < 60) return t("time.minutesAgo", { value: minutes, defaultValue: `${minutes}m ago` });
  const hours = Math.floor(minutes / 60);
  if (hours < 24) return t("time.hoursAgo", { value: hours, defaultValue: `${hours}h ago` });
  const days = Math.floor(hours / 24);
  return t("time.daysAgo", { value: days, defaultValue: `${days}d ago` });
}
