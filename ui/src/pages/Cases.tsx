    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("cases.statusPicker.ariaLabel")}
          onClick={(event) => {
  ...
  return (
    <span className="grid min-w-0 flex-1 items-center gap-2" style={{ gridTemplateColumns: caseTrailingGridTemplate(columns) }}>
      {columns.map((column) => {
        ...
        if (column === "key") {
          return row.key ? (
            <CaseCopyableToken
              key={column}
              value={row.key}
              label={t("cases.column.copyKey")}
              className="font-mono text-xs text-muted-foreground"
              stopPropagation
            />
          ) : (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground">{t("common.none")}</span>
          );
        }
        if (column === "title") {
          return (
                ...
                <span className="flex w-4 shrink-0 items-center text-muted-foreground">
                  {childCount > 0 ? (
                    <button
                      ...
                      aria-label={treeCollapsed ? t("cases.tree.expand", { title: row.title }) : t("cases.tree.collapse", { title: row.title })}
        ...
        if (column === "project") {
          return <span key={column} className="min-w-0 truncate text-xs text-muted-foreground">{projectName ?? t("cases.noProject")}</span>;
        }
        if (column === "parent") {
          return (
            <span key={column} className="min-w-0 truncate font-mono text-xs text-muted-foreground">
              {row.parentCaseId ? t("cases.column.parent") : t("common.none")}
            </span>
          );
        }
        if (column === "created") {
        ...
        return (
          <span key={column} className="text-right text-xs text-muted-foreground tabular-nums">
            {t("time.agoValue", { value: relativeTime(row.updatedAt) })}
          </span>
        );
      })}
    </span>
    ...
    >
      <span className="flex min-w-0 flex-1 flex-col gap-1 sm:contents">
        ...
        <span className="flex items-center gap-2 sm:hidden">
          ...
          <span className="text-xs text-muted-foreground sm:hidden">{t("time.agoValue", { value: relativeTime(row.updatedAt) })}</span>
        </span>
      </span>
...
function CaseColumnPicker({
  ...
  const { t } = useTranslation();
  return (
    <CaseToolbarButton icon={Columns3} title={t("toolbar.columns")} active={!sameStringSet([...visibleColumns], DEFAULT_CASE_COLUMNS)}>
      <PopoverContent align="end" className="w-(--sz-300px) p-1.5">
        <div className="px-2 pb-1 pt-1.5">
          <div className="text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
            {t("cases.columnPicker.desktopHeader")}
          </div>
          <div className="text-sm font-medium text-foreground">{t("cases.columnPicker.title")}</div>
        </div>
        <div className="space-y-0.5">
          {CASE_COLUMN_ORDER.map((column) => (
            ...
            >
              <span>{t(`cases.column.${column}`)}</span>
...
function CaseSortPicker({
  ...
  const { t } = useTranslation();
  return (
    <CaseToolbarButton icon={ArrowUpDown} title={t("toolbar.sort")} active={sortField !== "updated" || sortDir !== "desc"}>
...
function CasesEmptyHero() {
  ...
  return (
    <div className="mx-auto flex max-w-xl flex-col items-center gap-4 py-16 text-center">
      ...
      <h2 className="text-lg font-semibold">{t("cases.empty.heading")}</h2>
      ...
      <div className="w-full space-y-2 rounded-lg border border-border bg-muted/50 p-4 text-left">
        <p className="text-sm font-medium">{t("cases.empty.skillPrompt")}</p>
...
{slug, target_audience, publish_url} and key <release>/<slug>."`}
        ...
        <p className="text-xs text-muted-foreground">
          {t("cases.empty.skillReference", { path: "references/cases.md" })}
        </p>
      </div>
      <p className="text-xs text-muted-foreground">
        {t("cases.empty.experimentalNote", { flag: "enableCases" })}
      </p>
    </div>
  );
}
...
export function Cases() {
  ...
  const groupedRows = useMemo((): CaseGroupedRows[] => {
    ...
    for (const c of sorted) {
      ...
      else key = c.projectId ? projectName.get(c.projectId) ?? t("cases.projectFallback") : t("cases.noProject");
  ...
  if (viewState.search.trim()) activeFilters.push({ key: "search", label: t("cases.filter.chip.search"), value: viewState.search.trim() });
  if (viewState.typeFilters.length > 0) {
    activeFilters.push({ key: "type", label: t("cases.filter.chip.type"), value: viewState.typeFilters.join(", ") });
  }
  if (!usesDefaultStatusFilter) {
    activeFilters.push({
      key: "status",
      label: t("cases.filter.chip.status"),
      value: viewState.statusFilters.length === CASE_STATUSES.length
        ? t("cases.filter.chip.all")
        : viewState.statusFilters
  ...
  if (viewState.projectFilters.length > 0) {
    activeFilters.push({
      key: "project",
      label: t("cases.filter.chip.project"),
      value: viewState.projectFilters
        .map((projectId) => projectId === ALL ? t("cases.noProject") : projectName.get(projectId) ?? t("cases.filter.chip.projectFallback"))
        .join(", "),
    });
  }
  if (viewState.labelFilter !== ALL) {
    const name = (labelsQuery.data ?? []).find((l) => l.id === viewState.labelFilter)?.name ?? t("cases.filter.chip.labelFallback");
    activeFilters.push({ key: "label", label: t("cases.filter.chip.label"), value: name });
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="secondary">{t("cases.experimentalBadge")}</Badge>
        </div>
      </div>
      ...
      {noCasesAtAll ? (
        ...
        <>
          <div className="flex items-center justify-between gap-2 sm:gap-3">
            <div className="relative w-48 sm:w-64 md:w-80">
              ...
              <Input
                ...
                aria-label={t("cases.searchAriaLabel")}
            ...
            <div className="flex shrink-0 items-center gap-0.5 sm:gap-1">
              <Button
                ...
                title={viewState.treeView ? t("cases.treeViewToggle.flat") : t("cases.treeViewToggle.tree")}
                aria-label={viewState.treeView ? t("cases.treeViewToggle.flat") : t("cases.treeViewToggle.tree")}
              ...
              <CaseToolbarButton icon={Filter} title={t("toolbar.filters")} active={hasActiveFilters}>
                <PopoverContent align="end" className="w-72 p-3">
                  <div className="grid gap-3">
                    <FilterField label={t("cases.filter.label.type")}>
                      <div className="max-h-40 overflow-y-auto">
                        {distinctTypes.length === 0 ? (
                          <p className="px-1 py-1 text-xs text-muted-foreground">{t("cases.filter.noTypes")}</p>
                    ...
                    <FilterField label={t("cases.filter.label.status")}>
                      <div>
                        {STATUS_FILTER_OPTIONS.map((option) => (
                          <FilterCheckboxRow
                            key={option.value}
                    ...
                    <FilterField label={t("cases.filter.label.project")}>
                      <div className="max-h-40 overflow-y-auto">
                        <FilterCheckboxRow
                          label={t("cases.noProject")}
                    ...
                    <FilterField label={t("cases.filter.label.label")}>
                      <div className="max-h-40 overflow-y-auto">
                        <FilterCheckboxRow
                          label={t("cases.filter.allLabels")}
              ...
              <CaseToolbarButton icon={Layers} title={t("toolbar.group")} active={viewState.groupBy !== "type"}>
                <PopoverContent align="end" className="w-44 p-2">
                  {([
                    ["type", t("cases.group.type")],
                    ["project", t("cases.group.project")],
                    ["status", t("cases.group.status")],
                    ["none", t("cases.group.none")],
          ...
          {filtered.length === 0 ? (
            <EmptyState icon={SearchX} message={t("cases.empty.noMatch")} action={t("cases.filter.clear")} onAction={clearFilters} />
          ) : (
            <div ref={caseListRef}>
