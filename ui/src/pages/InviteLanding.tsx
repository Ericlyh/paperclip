...
function AwaitingJoinApprovalPanel({
  ...
  const approverLabel = invitedByUserName ?? t("inviteLanding.awaiting.adminFallback");
    ...
    <div className="min-h-screen bg-zinc-950 px-6 py-12 text-zinc-100">
      <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6" data-testid="invite-pending-approval">
        <div className="flex items-center gap-3">
          ...
          />
          <h1 className="text-lg font-semibold">{t("inviteLanding.awaiting.title", { company: companyDisplayName })}</h1>
        </div>
        <div className="mt-4 space-y-3">
          <p className="text-sm text-zinc-400">
            {t("inviteLanding.awaiting.body", { approver: approverLabel })}
          </p>
          <div className="border border-zinc-800 p-3">
            <p className="text-xs text-zinc-500 mb-1">{t("inviteLanding.awaiting.approvalPageLabel")}</p>
            <p className="text-sm text-zinc-200">{t("inviteLanding.awaiting.approvalPagePath")}</p>
          </div>
          <p className="text-sm text-zinc-400">
            {t("inviteLanding.awaiting.askThem", { path: t("inviteLanding.awaiting.approvalPagePath") })}
          </p>
          <p className="text-xs text-zinc-500">
            {t("inviteLanding.awaiting.refreshNote")}
          </p>
        </div>
        {claimSecret && claimApiKeyPath ? (
          <div className="mt-4 space-y-1 border border-zinc-800 p-3 text-xs text-zinc-400">
            <div className="text-zinc-200">{t("inviteLanding.awaiting.claimSecretLabel")}</div>
        ...
        {onboardingTextUrl ? (
          <div className="mt-4 text-xs text-zinc-400">
            {t("inviteLanding.awaiting.onboardingLabel")}: <span className="font-mono break-all">{onboardingTextUrl}</span>
          </div>
        ) : null}
      </div>
    </div>
...
export function InviteLandingPage() {
  ...
  const sessionLabel =
    ...
    t("inviteLanding.sessionFallback");
  ...
  const acceptMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isCheckingExistingMembership) {
        throw new Error(t("inviteLanding.errors.checkingAccess"));
      }
      if (isCurrentMember) {
        throw new Error(t("inviteLanding.errors.alreadyMember"));
      }
  ...
  const joinButtonLabel = useMemo(() => {
    if (!invite) return t("inviteLanding.buttons.continue");
    if (isCurrentMember) return t("inviteLanding.buttons.openCompany");
    if (invite.inviteType === "bootstrap_ceo") return t("inviteLanding.buttons.acceptInvite");
    if (showsAgentForm) return t("inviteLanding.buttons.submitRequest");
    return sessionQuery.data ? t("inviteLanding.buttons.acceptInvite") : t("inviteLanding.buttons.continue");
  }, [invite, isCurrentMember, sessionQuery.data, showsAgentForm, t]);

  if (!token) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">{t("inviteLanding.errors.invalidToken")}</div>;
  }
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="border border-border bg-card p-6" data-testid="invite-error">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t("inviteLanding.notAvailable.body")}
          </p>
        </div>
      </div>
    );
        ...
        <div className="border border-border bg-card p-6" data-testid="invite-error">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {inviteJoinRequestStatus === "rejected"
              ? t("inviteLanding.notAvailable.rejected")
              : t("inviteLanding.notAvailable.used")}
          </p>
        </div>
      </div>
    );
          ...
          <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6">
            <div className="flex items-center gap-3">
              <InviteCompanyLogo
                ...
                className="h-12 w-12 border border-zinc-800 rounded-none"
              />
              <h1 className="text-lg font-semibold">{t("inviteLanding.joinedCompany")}</h1>
            </div>
            <div className="mt-4">
        ...
        <div className="grid gap-6 lg:grid-cols-(--gtc-36)">
          <section className={`${panelClassName} space-y-6`}>
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                <p className="text-xs uppercase tracking-(--tracking-caps) text-zinc-500">
                  {t("inviteLanding.invitedTagline")}
                </p>
                <h1 className="mt-2 text-2xl font-semibold">
                  {invite.inviteType === "bootstrap_ceo" ? t("inviteLanding.bootstrapTitle") : t("inviteLanding.joinTitle", { company: companyDisplayName })}
                </h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-300">
                  {showsAgentForm
                    ? t("inviteLanding.descriptions.agentForm")
                    : requiresHumanAccount
                      ? t("inviteLanding.descriptions.requiresAccount")
                      : t("inviteLanding.descriptions.ready")}
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-zinc-500">{t("inviteLanding.summary.company")}</div>
                <div className="mt-1 text-sm text-zinc-100">{companyDisplayName}</div>
              </div>
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-zinc-500">{t("inviteLanding.summary.invitedBy")}</div>
                <div className="mt-1 text-sm text-zinc-100">{invitedByUserName ?? t("inviteLanding.summary.invitedByFallback")}</div>
              </div>
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-zinc-500">{t("inviteLanding.summary.requestedAccess")}</div>
                <div className="mt-1 text-sm text-zinc-100">
                  {showsAgentForm ? t("inviteLanding.summary.agentJoin") : requestedHumanRole ?? t("inviteLanding.summary.companyAccess")}
                </div>
              </div>
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-zinc-500">{t("inviteLanding.summary.inviteExpires")}</div>
            ...
            {inviteMessage ? (
              <div className="border border-amber-500/40 bg-amber-500/10 p-4">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-amber-200/80">{t("inviteLanding.messageFromInviter")}</div>
            ...
            {sessionQuery.data ? (
              <div className="border border-emerald-500/40 bg-emerald-500/10 p-4 text-sm text-emerald-50">
                {t("inviteLanding.signedInAs", { session: sessionLabel })}
              </div>
            ) : null}
          </section>
          ...
          <section className={`${panelClassName} h-fit`}>
            {showsAgentForm ? (
              <div className="space-y-4">
                <div>
                  <h2 className="text-lg font-semibold">{t("inviteLanding.agentForm.title")}</h2>
                  <p className="mt-1 text-sm text-zinc-400">
                    {t("inviteLanding.agentForm.subtitle", { company: companyDisplayName })}
                  </p>
                </div>
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("inviteLanding.agentForm.agentName")}</span>
                  <input
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("inviteLanding.agentForm.adapterType")}</span>
                  <select
                    className={fieldClassName}
                    value={adapterType}
                  ...
                  >
                    {joinAdapterOptions.map((type) => (
                      <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                        {getAdapterLabel(type)}{!ENABLED_INVITE_ADAPTERS.has(type) ? ` ${t("inviteLanding.agentForm.comingSoon")}` : ""}
                      </option>
                    ))}
                  </select>
                </label>
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("inviteLanding.agentForm.capabilities")}</span>
                  <textarea
                    className={fieldClassName}
                    rows={4}
                    value={capabilities}
                ...
                >
                  {acceptMutation.isPending ? t("inviteLanding.buttons.working") : joinButtonLabel}
                </Button>
              </div>
            ) : requiresHumanAccount ? (
              <div className="space-y-5">
                <div>
                  <h2 className="text-lg font-semibold">
                    {authMode === "sign_up" ? t("inviteLanding.authForm.titleCreate") : t("inviteLanding.authForm.titleSignIn")}
                  </h2>
                  <p className="mt-1 text-sm text-zinc-400">
                    {authMode === "sign_up"
                      ? t("inviteLanding.authForm.subtitleCreate", { company: companyDisplayName })
                      : t("inviteLanding.authForm.subtitleSignIn")}
                  </p>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {t("inviteLanding.authForm.createAccount")}
                  </button>
                  <button
                    type="button"
                    className={`${modeButtonBaseClassName} ${
                  ...
                  >
                    {t("inviteLanding.authForm.existingAccount")}
                  </button>
                </div>
                ...
                <form
                  ...
                  onSubmit={(event) => {
                    ...
                    if (!authCanSubmit) {
                      setAuthFeedback({ tone: "error", message: t("inviteLanding.authForm.fillAll") });
                      return;
                    }
                    authMutation.mutate();
                  }}
                ...
                >
                  {authMode === "sign_up" ? (
                    <label className="block text-sm" htmlFor="invite-name">
                      <span className="mb-1 block text-zinc-400">{t("inviteLanding.authForm.name")}</span>
                      <input
                        id="invite-name"
                        name="name"
                        className={fieldClassName}
                  ...
                  <label className="block text-sm" htmlFor="invite-email">
                    <span className="mb-1 block text-zinc-400">{t("inviteLanding.authForm.email")}</span>
                    <input
                      id="invite-email"
                      name="email"
                      type="email"
                  ...
                  <label className="block text-sm" htmlFor="invite-password">
                    <span className="mb-1 block text-zinc-400">{t("inviteLanding.authForm.password")}</span>
                    <input
                      id="invite-password"
                      name="password"
                      type="password"
                  ...
                  >
                    {authMutation.isPending
                      ? t("inviteLanding.buttons.working")
                      : authMode === "sign_in"
                        ? t("inviteLanding.buttons.signInContinue")
                        : t("inviteLanding.buttons.createAccountContinue")}
                  </Button>
                </form>

                <p className="text-xs leading-5 text-zinc-500">
                  {authMode === "sign_up"
                    ? t("inviteLanding.authForm.helperCreate")
                    : t("inviteLanding.authForm.helperSignIn")}
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                <div>
                  <h2 className="text-lg font-semibold">
                    {isCurrentMember
                      ? t("inviteLanding.acceptForm.alreadyMember")
                      : shouldAutoAcceptHumanInvite
                      ? t("inviteLanding.acceptForm.completing")
                      : invite.inviteType === "bootstrap_ceo"
                        ? t("inviteLanding.acceptForm.bootstrapTitle")
                        : t("inviteLanding.acceptForm.acceptTitle")}
                  </h2>
                  <p className="mt-1 text-sm text-zinc-400">
                    {shouldAutoAcceptHumanInvite
                      ? t("inviteLanding.acceptForm.granting", { company: companyDisplayName })
                      : isCurrentMember
                      ? t("inviteLanding.acceptForm.belongs", { company: companyDisplayName })
                      : t("inviteLanding.acceptForm.thisWill", {
                          action: invite.inviteType === "bootstrap_ceo"
                            ? t("inviteLanding.acceptForm.finishBootstrap")
                            : t("inviteLanding.acceptForm.grantOrComplete", { company: companyDisplayName }),
                        })}
                  </p>
                </div>
                {error ? <p className="text-xs text-red-400">{error}</p> : null}
                {shouldAutoAcceptHumanInvite ? (
                  <div className="text-sm text-zinc-400">
                    {acceptMutation.isPending ? t("inviteLanding.acceptForm.submitting") : t("inviteLanding.acceptForm.finishing")}
                  </div>
                ) : (
                  <Button
                    className="w-full rounded-none"
