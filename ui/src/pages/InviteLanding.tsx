      ...
      <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6" data-testid="invite-pending-approval">
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="border border-zinc-800 p-3">
            <p className="text-xs text-zinc-500 mb-1">{t("inviteLanding.Approval_page")}</p>
            <p className="text-sm text-zinc-200">{t("inviteLanding.Company_Settings_Members")}</p>
          </div>
          <p className="text-sm text-zinc-400">
            Ask them to visit <span className="text-zinc-200">{t("inviteLanding.Company_Settings_Members")}</span> to approve your request.
          </p>
        ...
        {claimSecret && claimApiKeyPath ? (
          <div className="mt-4 space-y-1 border border-zinc-800 p-3 text-xs text-zinc-400">
            <div className="text-zinc-200">{t("inviteLanding.Claim_secret")}</div>
...
export function InviteLandingPage() {
  ...
  if (!token) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">{t("inviteLanding.Invalid_invite_token")}</div>;
  }

  if (inviteQuery.isLoading || healthQuery.isLoading || sessionQuery.isLoading) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">{t("inviteLanding.Loading_invite")}</div>;
  }

  if (isCheckingExistingMembership) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">{t("inviteLanding.Checking_your_access")}</div>;
  }
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="border border-border bg-card p-6" data-testid="invite-error">
          <h1 className="text-lg font-semibold">{t("inviteLanding.Invite_not_available")}</h1>
  ...
  if (
    ...
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">{t("inviteLanding.Opening_company")}</div>;
  }
    ...
    return (
      <div className="min-h-screen bg-zinc-950 px-6 py-12 text-zinc-100">
        <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6">
          <h1 className="text-lg font-semibold">{t("inviteLanding.Bootstrap_complete")}</h1>
          <div className="mt-4">
            <Button asChild className="rounded-none">
              <Link to="/">{t("inviteLanding.Open_board")}</Link>
            </Button>
          </div>
        </div>
      </div>
          ...
          <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6">
            <div className="flex items-center gap-3">
              <InviteCompanyLogo
                ...
                className="h-12 w-12 border border-zinc-800 rounded-none"
              />
              <h1 className="text-lg font-semibold">{t("inviteLanding.You_joined_the_company")}</h1>
            </div>
            <div className="mt-4">
        ...
        <div className="grid gap-6 lg:grid-cols-(--gtc-36)">
          <section className={`${panelClassName} space-y-6`}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-zinc-500">{t("inviteLanding.Invited_by")}</div>
                <div className="mt-1 text-sm text-zinc-100">{invitedByUserName ?? "Paperclip board"}</div>
              </div>
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-zinc-500">{t("inviteLanding.Requested_access")}</div>
                <div className="mt-1 text-sm text-zinc-100">
                  {showsAgentForm ? "Agent join request" : requestedHumanRole ?? "Company access"}
                </div>
              </div>
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-zinc-500">{t("inviteLanding.Invite_expires")}</div>
            ...
            {inviteMessage ? (
              <div className="border border-amber-500/40 bg-amber-500/10 p-4">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-amber-200/80">{t("inviteLanding.Message_from_inviter")}</div>
          ...
          <section className={`${panelClassName} h-fit`}>
            {showsAgentForm ? (
              <div className="space-y-4">
                <div>
                  <h2 className="text-lg font-semibold">{t("inviteLanding.Submit_agent_details")}</h2>
                  <p className="mt-1 text-sm text-zinc-400">
                    This invite will create an approval request for a new agent in {companyDisplayName}.
                  </p>
                </div>
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("inviteLanding.Agent_name")}</span>
                  <input
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("inviteLanding.Adapter_type")}</span>
                  <select
                    className={fieldClassName}
                    value={adapterType}
