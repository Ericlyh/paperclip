...
export function RuntimeProvisionStatusValue({
  ...
  if (status.kind === "eager") {
    return (
      <span className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.Eager_provisioned_during_workspace_setup")}</span>
    );
  }
  ...
  return (
    <>
      <Card className="rounded-none">
        <CardHeader>
          <CardTitle>{t("executionWorkspaceDetail.Workspace_routines")}</CardTitle>
          <CardDescription>
        ...
        <CardContent>
          {isLoading ? (
            <p className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.Loading_routines")}</p>
          ) : error ? (
...
export function ExecutionWorkspaceDetail() {
  ...
  if (workspaceQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.Loading_workspace")}</p>;
      ...
      <div className="space-y-4 overflow-hidden sm:space-y-6">
        ...
        {activeTab === "services" ? (
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("executionWorkspaceDetail.Workspace_settings")}</CardTitle>
                <CardDescription>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  ...
                  <Field label={t("executionWorkspaceDetail.Workspace_name")}>
                    <Input
                      value={form.name}
                      onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                      placeholder={t("executionWorkspaceDetail.Execution_workspace_name")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspaceDetail.Source_control")}</div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={t("executionWorkspaceDetail.Branch_name")} hint="Useful for isolated worktrees">
                      <Input
                        className="font-mono"
                    ...
                    <Field label={t("executionWorkspaceDetail.Base_ref")}>
                      <Input
                        className="font-mono"
                        value={form.baseRef}
                        onChange={(event) => setForm((current) => current ? { ...current, baseRef: event.target.value } : current)}
                        placeholder={t("executionWorkspaceDetail.origin_main")}
                      />
                    </Field>
                  </div>

                  <Field label={t("executionWorkspaceDetail.Repo_URL")}>
                    <Input
                      value={form.repoUrl}
                      onChange={(event) => setForm((current) => current ? { ...current, repoUrl: event.target.value } : current)}
                      placeholder={t("executionWorkspaceDetail.https_github_com_org_repo")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <Field label={t("executionWorkspaceDetail.Working_directory")}>
                    <Input
                      className="font-mono"
                      value={form.cwd}
                      onChange={(event) => setForm((current) => current ? { ...current, cwd: event.target.value } : current)}
                      placeholder={t("executionWorkspaceDetail.absolute_path_to_workspace")}
                    />
                  </Field>

                  <Field label={t("executionWorkspaceDetail.Provider_path_ref")}>
                    <Input
                      className="font-mono"
                      value={form.providerRef}
                      onChange={(event) => setForm((current) => current ? { ...current, providerRef: event.target.value } : current)}
                      placeholder={t("executionWorkspaceDetail.path_to_worktree_or_provider_ref")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspaceDetail.Lifecycle_commands")}</div>
                  <Field label={t("executionWorkspaceDetail.Provision_command")} hint="Runs when Paperclip prepares this execution workspace">
                    <Textarea
                      className="min-h-20 font-mono"
                      value={form.provisionCommand}
                      onChange={(event) => setForm((current) => current ? { ...current, provisionCommand: event.target.value } : current)}
                      placeholder={t("executionWorkspaceDetail.bash_scripts_provision_worktree_sh")}
                    />
                  </Field>

                  <Field
                    label={t("executionWorkspaceDetail.Runtime_provision_command")}
                  ...
                  >
                    <Textarea
                      ...
                      placeholder={t("executionWorkspaceDetail.bash_scripts_provision_worktree_runtime_")}
                    />
                  </Field>

                  <Field label={t("executionWorkspaceDetail.Teardown_command")} hint="Runs when the execution workspace is archived or cleaned up">
                    <Textarea
                      className="min-h-20 font-mono"
                      value={form.teardownCommand}
                      onChange={(event) => setForm((current) => current ? { ...current, teardownCommand: event.target.value } : current)}
                      placeholder={t("executionWorkspaceDetail.bash_scripts_teardown_worktree_sh")}
                    />
                  </Field>

                  <Field label={t("executionWorkspaceDetail.Cleanup_command")} hint="Workspace-specific cleanup before teardown">
                    <Textarea
                      className="min-h-16 font-mono"
                      value={form.cleanupCommand}
                      onChange={(event) => setForm((current) => current ? { ...current, cleanupCommand: event.target.value } : current)}
                      placeholder={t("executionWorkspaceDetail.pkill_f_vite_true")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspaceDetail.Runtime_config")}</div>
                  ...
                  <details className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <summary className="cursor-pointer text-sm font-medium">{t("executionWorkspaceDetail.Advanced_runtime_JSON")}</summary>
                    ...
                    <div className="mt-3">
                      <Field label={t("executionWorkspaceDetail.Workspace_commands_JSON")} hint="Legacy `services` arrays still work, but `commands` supports both services and jobs.">
                        <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          />
                          <label htmlFor="inherit-runtime-config">{t("executionWorkspaceDetail.Inherit_project_workspace_runtime_config")}</label>
                        </div>
                        <Textarea
                  ...
                  {configuredRuntimeServicePorts.length > 0 ? (
                    <div className="space-y-3 rounded-md border border-border bg-muted/20 p-4">
                      <div>
                        <div className="text-sm font-medium">{t("executionWorkspaceDetail.Service_ports")}</div>
              ...
              <div className="mt-6 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                ...
                {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.No_unsaved_changes")}</p> : null}
              </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("executionWorkspaceDetail.Workspace_context")}</CardTitle>
                <CardDescription>{t("executionWorkspaceDetail.Linked_objects_and_relationships")}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label="Project">
                {project ? <Link to={`/projects/${projectRef}`} className="hover:underline">{project.name}</Link> : <MonoValue value={workspace.projectId} />}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.Project_workspace")}>
              ...
              <DetailRow label={t("executionWorkspaceDetail.Source_task")}>
                {sourceIssue ? (
              ...
              <DetailRow label={t("executionWorkspaceDetail.Derived_from")}>
                {derivedWorkspace ? (
              ...
              <DetailRow label={t("executionWorkspaceDetail.Runtime_provisioning")}>
                <RuntimeProvisionStatusValue
                  status={runtimeProvisionStatus}
                  onViewLogs={() => handleTabChange("runtime_logs")}
                />
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.Workspace_ID")}>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("executionWorkspaceDetail.Concrete_location")}</CardTitle>
                <CardDescription>{t("executionWorkspaceDetail.Paths_and_refs")}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("executionWorkspaceDetail.Working_dir")}>
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : "None"}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.Provider_ref")}>
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : "None"}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.Repo_URL")}>
              ...
              <DetailRow label={t("executionWorkspaceDetail.Base_ref")}>
              ...
              <DetailRow label={t("executionWorkspaceDetail.Last_used")}>{formatDateTime(workspace.lastUsedAt)}</DetailRow>
          ...
          <Card className="rounded-none">
            <CardHeader>
              <CardTitle>{t("executionWorkspaceDetail.Runtime_and_cleanup_logs")}</CardTitle>
              <CardDescription>{t("executionWorkspaceDetail.Recent_operations")}</CardDescription>
            </CardHeader>
            <CardContent>
            {workspaceOperationsQuery.isLoading ? (
              <p className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.Loading_workspace_operations")}</p>
              ...
              <p className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.No_workspace_operations_have_been_record")}</p>
            )}
            </CardContent>
          </Card>
        ) : activeTab === "issues" ? (
          <div className="space-y-6">
            {workspace.projectWorkspaceId ? (
              <SummarySlotCard
                ...
                title={t("executionWorkspaceDetail.Workspace_summary")}
          ...
          <Card>
            <CardContent className="py-6 text-sm text-muted-foreground">{t("executionWorkspaceDetail.Loading_workspace_plugin")}</CardContent>
          </Card>
