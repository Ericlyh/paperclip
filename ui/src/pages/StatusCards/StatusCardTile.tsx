...
export function StatusCardTile({
  ...
  // t intentionally not memoized — only used inside inline render. If extracted to useMemo, add t to deps.
  ...
  const freshnessLabel = card.lastGeneratedAt ? t("time.updatedAgo", { value: relativeTime(card.lastGeneratedAt) }) : t("statusCards.noSummaryYet");
    ...
    >
        ...
        <div onClick={(event) => event.stopPropagation()}>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="-mr-1 -mt-1 h-7 w-7 text-muted-foreground" aria-label={t("statusCards.cardActionsAria")}>
                <MoreHorizontal className="h-4 w-4" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              <DropdownMenuItem onSelect={onOpen}>{t("statusCards.openDetail")}</DropdownMenuItem>
              <DropdownMenuItem onSelect={onRefresh} disabled={refreshPending || lifecycle === "updating"}>
                {t("statusCards.refreshNow")}
              </DropdownMenuItem>
              {(lifecycle === "compiling" && !setupRunning) || lifecycle === "error" ? (
                <DropdownMenuItem onSelect={onRecompile} disabled={recompilePending}>
                  {t("statusCards.runNow")}
                </DropdownMenuItem>
              ) : null}
              <DropdownMenuItem onSelect={onEditInterest}>{t("statusCards.editInterestSettings")}</DropdownMenuItem>
              <DropdownMenuItem onSelect={onOpenDebug}>{t("statusCards.queryDebug")}</DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onSelect={onArchive} variant="destructive">
                {t("statusCards.archive")}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      ...
      <div className="px-4 pt-2">
        {lifecycle === "compiling" ? (
          <div className="rounded-md bg-muted px-3 py-2 text-xs text-foreground" role="status" aria-live="polite">
            <div className="flex items-center gap-2">
              ...
              <span>{setupRunning ? t("statusCards.settingUp") : t("statusCards.setupFailed")}</span>
            </div>
            <p className="mt-1 line-clamp-2 text-muted-foreground">{t("statusCards.interestPromptQuote", { value: card.interestPrompt })}</p>
            {setupRunning ? (
              ...
              >
                ...
                {recompilePending ? t("statusCards.starting") : t("statusCards.runNow")}
              </button>
            )}
          </div>
        ) : null}
              ...
              <span className="min-w-0 flex-1 truncate" title={draftStream.statusLine ?? undefined}>
                {draftStream.statusLine
                  ?? (card.pendingChangeCount > 0
                    ? t("statusCards.integratingChanges", { count: card.pendingChangeCount })
                    : t("statusCards.updatingNow"))}
              </span>
        ...
        {lifecycle === "stale" ? (
          ...
          >
            <span>
              {t("statusCards.changesSinceUpdate", { count: card.pendingChangeCount })}
            </span>
            <span className="shrink-0 font-medium text-amber-700 dark:text-amber-400">{t("common.refresh")}</span>
          </button>
        ) : null}
        ...
        {lifecycle === "error" ? (
          <div className="flex items-center justify-between gap-2 rounded-md border border-destructive/40 bg-destructive/5 px-3 py-1.5 text-xs">
            <span className="flex items-center gap-1.5 text-destructive">
              ...
              {t("statusCards.lastUpdateFailed")}
            </span>
        ...
        {lifecycle === "paused_budget" || lifecycle === "paused_hours" ? (
          <div className="flex items-center gap-2 rounded-md border border-orange-500/40 bg-orange-500/5 px-3 py-1.5 text-xs text-foreground">
            ...
            <span>
              {t(lifecycle === "paused_budget" ? "statusCards.pausedBudget" : "statusCards.pausedHours")}
            </span>
          </div>
        ) : null}
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-hidden px-4 pt-2">
        {lifecycle === "error" && card.summaryBody ? (
          <p className="mb-1 text-(length:--text-micro) text-muted-foreground">{t("statusCards.showingLastGoodSummary")}</p>
        ) : null}
        {hasSummary ? (
          <MarkdownBody className="text-xs leading-6 text-foreground [&_p]:my-0.5">{card.summaryBody!}</MarkdownBody>
        ) : lifecycle === "compiling" ? (
          <p className="text-xs text-muted-foreground">
            {t("statusCards.compileHint")}
          </p>
          ...
          <p className="text-xs text-muted-foreground">{t("statusCards.noSummary")}</p>
        )}
      </div>
      ...
      <div className="mt-auto flex items-center justify-between gap-2 border-t border-border px-4 py-2.5">
        <span className="truncate text-(length:--text-micro) text-muted-foreground">
          {lifecycle === "compiling" ? (
            t("statusCards.compileFooter")
          ) : (
            <>
              {freshnessLabel}{t("common.dotSeparator")}{policyLabel}
              {tokensLabel ? `${t("common.dotSeparator")}${tokensLabel}` : ""}
              {costLabel ? `${t("common.dotSeparator")}${costLabel}` : ""}
            </>
          )}
        </span>
        ...
        {lifecycle === "fresh" || lifecycle === "paused_budget" || lifecycle === "paused_hours" ? (
          <Button
            ...
            onClick={stopClick(onRefresh)}
            disabled={refreshPending}
            aria-label={t("statusCards.refreshCardAria")}
          >
