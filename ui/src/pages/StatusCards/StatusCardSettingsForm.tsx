...
// Labels live in `statusCardSettingsForm.trigger.<key>` and are resolved at the
// JSX call site — this array is module-level, where `useTranslation()` cannot run.
const TRIGGER_ROWS: { key: TriggerKey; noisy?: boolean }[] = [
  { key: "statusTransitions" },
  { key: "membershipChanges" },
  { key: "humanComments" },
  { key: "assigneeChanges" },
  { key: "anyUpdate", noisy: true },
];
      ...
      <section className="space-y-2">
        ...
        <div className="space-y-2">
          <RadioRow
            ...
            title={t("statusCardSettingsForm.manualOnly")}
            badge={
              <span className="ml-auto rounded-full bg-muted px-2 py-0.5 text-(length:--text-nano) font-medium uppercase tracking-wide text-muted-foreground">
                {t("statusCardSettingsForm.default")}
              </span>
            }
          ...
          <RadioRow
            ...
            title={t("statusCardSettingsForm.onASchedule")}
          ...
          >
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              >
                <SelectTrigger size="sm" className="w-28" aria-label={t("statusCardSettingsForm.checkInterval")}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {INTERVAL_OPTIONS.map((minutes) => (
                    <SelectItem key={minutes} value={String(minutes)}>
                      {t("statusCardSettingsForm.minutesShort", { minutes })}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <RadioRow
            ...
            title={t("statusCardSettingsForm.asSoonAsChanges")}
          ...
          >
            <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              ...
              >
                <SelectTrigger size="sm" className="w-24" aria-label={t("statusCardSettingsForm.debounce")}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {DEBOUNCE_OPTIONS.map((seconds) => (
                    <SelectItem key={seconds} value={String(seconds)}>
                      {t("statusCardSettingsForm.secondsShort", { seconds })}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              ...
              <Input
                ...
                aria-label={t("statusCardSettingsForm.maxUpdatesPerHour")}
              />
      ...
      {autoUpdating ? (
        <Collapsible className="rounded-md border border-border">
          ...
          <CollapsibleContent className="space-y-6 border-t border-border px-3 py-3">
            <section className="space-y-2">
              <h3 className="text-sm font-semibold">{t("statusCardSettingsForm.countAsAChange")}</h3>
              <div className="space-y-2">
                {TRIGGER_ROWS.map((row) => (
                  <label key={row.key} className="flex items-start gap-2.5 text-sm">
                    <Checkbox
                      ...
                      className="mt-0.5"
                      aria-label={t(`statusCardSettingsForm.trigger.${row.key}`)}
                    />
                    <span className={cn(row.noisy && "text-muted-foreground")}>
                      {t(`statusCardSettingsForm.trigger.${row.key}`)}
                    </span>
                  </label>
                ))}
              </div>
            ...
            <section className="space-y-3">
              ...
              <label className="flex items-start gap-2.5 text-sm">
                <Checkbox checked={Boolean(activeHours)} onCheckedChange={(checked) => setActiveHoursEnabled(Boolean(checked))} className="mt-0.5" aria-label={t("statusCardSettingsForm.limitToActiveHours")} />
                <span>{t("statusCardSettingsForm.onlyAutoUpdateDuringActiveHours")}</span>
              </label>
              {activeHours ? (
                <div className="flex flex-wrap items-center gap-2 pl-6 text-sm">
                  <Input
                    ...
                    className="h-8 w-32"
                    aria-label={t("statusCardSettingsForm.activeHoursStart")}
                  />
                  ...
                  <Input
                    ...
                    className="h-8 w-32"
                    aria-label={t("statusCardSettingsForm.activeHoursEnd")}
                  />
                  <Input
                    value={activeHours.timezone}
                    onChange={(event) => setPolicy({ activeHours: { ...activeHours, timezone: event.target.value } })}
                    className="h-8 w-40"
                    placeholder={t("statusCardSettingsForm.timezone")}
                    aria-label={t("statusCardSettingsForm.activeHoursTimezone")}
                  />
                </div>
              ) : null}
              <div className="flex flex-wrap items-center gap-2 text-sm">
                ...
                <Input
                  ...
                  aria-label={t("statusCardSettingsForm.dailyTokenCap")}
                />
              </div>
            </section>
          </CollapsibleContent>
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <Tooltip>
          ...
          <TooltipContent className="max-w-(--sz-18rem) text-left">
            ...
            <p className="mt-1 opacity-80">{t("statusCardSettingsForm.roughEstimate")}</p>
          </TooltipContent>
        </Tooltip>
      </div>
    </div>
