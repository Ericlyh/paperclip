...
export function CompanyAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("companyAccess.breadcrumb") },
    ]);
  ...
  const updateMemberMutation = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toasts.memberUpdateFailed"),
        body: error instanceof Error ? error.message : t("companyAccess.toasts.memberUpdateUnknownError"),
        tone: "error",
      });
    },
  });
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("companyAccess.toasts.joinRequestApproved"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toasts.joinRequestApproveFailed"),
        body: error instanceof Error ? error.message : t("companyAccess.toasts.memberUpdateUnknownError"),
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("companyAccess.toasts.joinRequestRejected"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toasts.joinRequestRejectFailed"),
        body: error instanceof Error ? error.message : t("companyAccess.toasts.memberUpdateUnknownError"),
        tone: "error",
      });
    },
  ...
  const archiveMemberMutation = useMutation({
        ...
        body:
          result.reassignedIssueCount > 0
            ? (result.reassignedIssueCount === 1
                ? t("companyAccess.toasts.reassignedTasks", { count: result.reassignedIssueCount })
                : t("companyAccess.toasts.reassignedTasksPlural", { count: result.reassignedIssueCount }))
            : undefined,
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toasts.memberRemovalFailed"),
        body: error instanceof Error ? error.message : t("companyAccess.toasts.memberUpdateUnknownError"),
        tone: "error",
      });
    },
    ...
    <div className="max-w-6xl space-y-8">
      <div className="space-y-3">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("companyAccess.subtitle", { companyName: selectedCompany?.name ?? "" })}
        </p>
        <div className="rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground">
          {t("companyAccess.coreScopeNote")}
        </div>
      </div>
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("companyAccess.sections.humans.title")}</h2>
          </div>
          <p className="max-w-3xl text-sm text-muted-foreground">
            {t("companyAccess.sections.humans.description")}
          </p>
        </div>
        ...
        {access?.canApproveJoinRequests && pendingHumanJoinRequests.length > 0 ? (
          <div className="space-y-3 rounded-xl border border-border px-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 className="text-sm font-semibold">{t("companyAccess.sections.pendingJoins.title")}</h3>
                <p className="text-sm text-muted-foreground">
                  {t("companyAccess.sections.pendingJoins.description")}
                </p>
              </div>
              <Badge variant="outline">{t("companyAccess.sections.pendingJoins.pending", { count: pendingHumanJoinRequests.length })}</Badge>
            </div>
            <div className="space-y-3">
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  title={
                    ...
                    t("companyAccess.sections.pendingJoins.unknownRequester")
                  }
                  subtitle={
                    request.requesterUser?.email ||
                    request.requestEmailSnapshot ||
                    request.requestingUserId ||
                    t("companyAccess.sections.pendingJoins.noEmail")
                  }
                  context={
                    request.invite
                      ? (request.invite.humanRole
                          ? t("companyAccess.sections.pendingJoins.inviteWithRole", { joinType: request.invite.allowedJoinTypes, role: request.invite.humanRole })
                          : `${request.invite.allowedJoinTypes} join invite`)
                      : t("companyAccess.sections.pendingJoins.inviteUnavailable")
                  }
                  detail={t("companyAccess.sections.pendingJoins.submitted", { date: new Date(request.createdAt).toLocaleString() })}
                  approveLabel={t("companyAccess.sections.pendingJoins.approve")}
                  rejectLabel={t("companyAccess.sections.pendingJoins.reject")}
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          <div className="grid grid-cols-(--gtc-24) gap-3 border-b border-border px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
            <div>{t("companyAccess.sections.table.userAccount")}</div>
            <div>{t("companyAccess.sections.table.role")}</div>
            <div>{t("companyAccess.sections.table.status")}</div>
            <div className="text-right">{t("companyAccess.sections.table.action")}</div>
          </div>
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">{t("companyAccess.sections.table.empty")}</div>
          ) : (
            members.map((member) => {
                ...
                >
                  ...
                  <div className="text-sm">
                    {member.membershipRole
                      ...
                      : t("companyAccess.sections.table.unset")}
                  </div>
                  <div>
                    <Badge variant={member.status === "active" ? "secondary" : member.status === "suspended" ? "destructive" : "outline"}>
                      {member.status === "active" ? t("companyAccess.sections.status.active") : member.status === "suspended" ? t("companyAccess.sections.status.suspended") : member.status.replace("_", " ")}
                    </Badge>
                  </div>
                  <div className="space-y-1 text-right">
                    <div className="flex justify-end gap-2">
                      <Button size="sm" variant="outline" onClick={() => setEditingMemberId(member.id)}>
                        {t("companyAccess.sections.table.edit")}
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
                      ...
                      >
                        ...
                        {t("companyAccess.sections.table.remove")}
                      </Button>
                    </div>
                    {removalReason ? (
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{t("companyAccess.editDialog.title")}</DialogTitle>
            <DialogDescription>
              {t("companyAccess.editDialog.description", { name: editingMember?.user?.name || editingMember?.user?.email || editingMember?.principalId || "" })}
            </DialogDescription>
          </DialogHeader>
          {editingMember && (
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("companyAccess.editDialog.companyRole")}</span>
                  <select
                  ...
                  >
                    <option value="">{t("companyAccess.sections.table.unset")}</option>
                ...
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("companyAccess.editDialog.membershipStatus")}</span>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2"
                    value={draftStatus}
                    onChange={(event) =>
                  ...
                  >
                    <option value="active">{t("companyAccess.sections.status.active")}</option>
                    <option value="pending">{t("companyAccess.sections.status.pending")}</option>
                    <option value="suspended">{t("companyAccess.sections.status.suspended")}</option>
                  </select>
                </label>
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditingMemberId(null)}>
              {t("companyAccess.editDialog.cancel")}
            </Button>
            <Button
              onClick={() => {
                if (!editingMember) return;
            ...
            >
              {updateMemberMutation.isPending ? t("companyAccess.editDialog.saving") : t("companyAccess.editDialog.save")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!removingMember} onOpenChange={(open) => !open && setRemovingMemberId(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("companyAccess.removeDialog.title")}</DialogTitle>
            <DialogDescription>
              {t("companyAccess.removeDialog.description", { name: memberDisplayName(removingMember) ?? t("companyAccess.fallbackName") })}
            </DialogDescription>
          </DialogHeader>
          {removingMember && (
            <div className="space-y-5">
              <div className="rounded-lg border border-border px-3 py-3">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ? t("companyAccess.removeDialog.checking")
                    : (assignedIssues.length === 1
                        ? t("companyAccess.removeDialog.openAssignedTasks", { count: assignedIssues.length })
                        : t("companyAccess.removeDialog.openAssignedTasksPlural", { count: assignedIssues.length }))}
                </div>
              </div>

              {assignedIssues.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-sm font-medium">{t("companyAccess.removeDialog.taskReassignment")}</div>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                    value={reassignmentTarget}
                    onChange={(event) => setReassignmentTarget(event.target.value)}
                  >
                    <option value="__unassigned">{t("companyAccess.removeDialog.leaveUnassigned")}</option>
                    {activeReassignmentUsers.length > 0 ? (
                      <optgroup label={t("companyAccess.removeDialog.humansGroup")}>
                    ...
                    {activeReassignmentAgents.length > 0 ? (
                      <optgroup label={t("companyAccess.removeDialog.agentsGroup")}>
                    ...
                    {assignedIssues.length > 6 ? (
                      <div className="px-3 py-2 text-sm text-muted-foreground">
                        {assignedIssues.length - 6 === 1
                          ? t("companyAccess.removeDialog.moreTasks", { count: assignedIssues.length - 6 })
                          : t("companyAccess.removeDialog.moreTasksPlural", { count: assignedIssues.length - 6 })}
                      </div>
                    ) : null}
                  </div>
                </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setRemovingMemberId(null)}>
              {t("companyAccess.removeDialog.cancel")}
            </Button>
            <Button
              variant="destructive"
              onClick={() => {
            ...
            >
              {archiveMemberMutation.isPending ? t("companyAccess.removeDialog.removing") : t("companyAccess.removeDialog.remove")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export function CompanyAccessLegacyRoute() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("sidebar.settings"), href: "/company/settings" },
      { label: t("companyAccess.advancedPermissions.legacyBreadcrumb") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  if (isLoading) {
    return <div className="text-sm text-muted-foreground">{t("companyAccess.advancedPermissions.loading")}</div>;
  }
    ...
    <div className="max-w-2xl space-y-5">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("companyAccess.advancedPermissions.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("companyAccess.advancedPermissions.subtitle")}
        </p>
      </div>

      <div className="space-y-4 rounded-xl border border-border px-5 py-5">
        <div className="space-y-2">
          <h2 className="text-sm font-semibold">{t("companyAccess.advancedPermissions.unavailableTitle")}</h2>
          <p className="text-sm text-muted-foreground">
            {t("companyAccess.advancedPermissions.unavailableBody")}
          </p>
          {errorMessage ? (
            <p className="text-sm text-destructive">{t("companyAccess.advancedPermissions.pluginError", { message: errorMessage })}</p>
          ) : null}
        </div>
        <div className="flex flex-wrap gap-2">
          <Button asChild>
            <Link to="/company/settings/members">{t("companyAccess.advancedPermissions.openMembers")}</Link>
          </Button>
          <Button asChild variant="outline">
            <Link to="/company/settings/invites">{t("companyAccess.advancedPermissions.openInvites")}</Link>
          </Button>
        </div>
      </div>
    </div>
