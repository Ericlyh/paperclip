...
export function CliAuthPage() {
  ...
  if (challenge.status === "approved") {
    return (
      <div className="mx-auto max-w-xl py-10">
        <Card className="block p-6">
          <h1 className="text-xl font-semibold">{t("cliAuth.approvedTitle")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("cliAuth.approvedBody")}
          </p>
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <Card className="block p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t("cliAuth.expiredOrCancelledBody")}
          </p>
        </Card>
      </div>
    );
      ...
      <div className="mx-auto max-w-xl py-10">
        <Card className="block p-6">
          ...
          <Button asChild className="mt-4">
            <Link to={`/auth?next=${encodeURIComponent(currentPath)}`}>{t("cliAuth.signInOrCreate")}</Link>
          </Button>
        </Card>
      </div>
    );
      ...
      <Card className="block p-6">
        ...
        <div className="mt-5 space-y-3 text-sm">
          ...
          <div>
            ...
            <div className="text-foreground">{challenge.clientName ?? t("cliAuth.defaultClientName")}</div>
          </div>
          <div>
            <div className="text-muted-foreground">{t("cliAuth.requestedAccessLabel")}</div>
            <div className="text-foreground">
              {challenge.requestedAccess === "instance_admin_required" ? t("cliAuth.requestedAccessInstanceAdmin") : t("cliAuth.requestedAccessBoard")}
            </div>
          </div>
        ...
        {(approveMutation.error || cancelMutation.error) && (
          <p className="mt-4 text-sm text-destructive">
            {(approveMutation.error ?? cancelMutation.error) instanceof Error
              ...
              : t("cliAuth.approveErrorFallback")}
          </p>
        )}

        {!challenge.canApprove && (
          <p className="mt-4 text-sm text-destructive">
            {t("cliAuth.adminRequiredBody")}
          </p>
        )}
