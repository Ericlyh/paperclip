...
import { useDateRange, PRESET_KEYS, presetLabel } from "../hooks/useDateRange";
    ...
    <Card>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={t("costs.financeEventsInRange", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("costs.credits")}
          value={formatCents(creditCents)}
          subtitle={t("costs.refundsOffsetsCredits")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costs.net")}
          value={formatCents(netCents)}
          subtitle={t("costs.financeNetDescription")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costs.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costs.inferenceLedgerDescription")}
          icon={Coins}
        />
      </CardContent>
    </Card>
      ...
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={t("costs.inferenceTokensSubtitle", { count: inferenceTokenTotal, formatted: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("costs.metricBudgetLabel")}
            ...
            <MetricTile
              ...
              subtitle={t("costs.financeEventsEstimated", { amount: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
            ...
            <>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costs.inferenceLedgerBudget", { amount: formatCents(spendData.summary.budgetCents) })
                            : t("costs.unlimitedBudget")}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("costs.inferenceLedgerUsageLabel")}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costs.utilizationSubtitle", { percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-32)">
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("costs.tokenBreakdown", { input: formatTokens(row.inputTokens + row.cachedInputTokens), output: formatTokens(row.outputTokens) })}
                                </div>
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {t("costs.tokensAbbrev", { value: formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens) })}
                                        </div>
                                      </div>
                                    </div>
                                  );
                ...
                <div className="space-y-4">
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs.noFinanceEventsHint")} />
                </div>
              </div>
            </>
          )}
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t(`costs.${scopeType}BudgetsLabel`)}</h2>
                        <p className="text-sm text-muted-foreground">
                          {t(
                            scopeType === "company"
                              ? "costs.companyWideMonthlyPolicy"
                              : scopeType === "agent"
                                ? "costs.agentMonthlyPolicies"
                                : "costs.lifetimeSpendPolicies"
                          )}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>{t("costs.financeLedgerDescription")}</CardDescription>
                    </CardHeader>
