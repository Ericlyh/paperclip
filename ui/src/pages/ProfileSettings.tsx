...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("settings.profileSettings.breadcrumbSettings"), href: "/company/settings" },
      { label: t("settings.profileSettings.breadcrumbInstanceSettings"), href: "/company/settings/instance/general" },
      { label: t("settings.profileSettings.breadcrumbProfile") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("settings.profileSettings.board");
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.profileSettings.failedToUpdateProfile"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("settings.profileSettings.selectCompanyFirst"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.profileSettings.failedToUploadAvatar"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.profileSettings.failedToRemoveAvatar"));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("settings.profileSettings.loadingProfile")}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("settings.profileSettings.failedToLoadProfile")}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("settings.profileSettings.board");
  ...
  const uploadHint = selectedCompany
    ? t("settings.profileSettings.uploadHintCompany", { company: selectedCompany.name })
    : t("settings.profileSettings.uploadHintNoCompany");
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("settings.profileSettings.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("settings.profileSettings.subtitle")}
        </p>
      </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-(--rad-24) border border-border/70 bg-background/92 p-5 shadow-(--shadow-extract-18) backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("settings.profileSettings.changePhoto") : t("settings.profileSettings.uploadPhoto")}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
                    ...
                    >
                      ...
                      {t("settings.profileSettings.remove")}
                    </Button>
                  ) : null}
                </div>
              </div>
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? t("settings.profileSettings.noEmail")}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("settings.profileSettings.clickAvatarHint")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("settings.profileSettings.displayName")}</Label>
            <Input
              id="profile-name"
              value={name}
              onChange={(event) => setName(event.target.value)}
              maxLength={120}
              placeholder={t("settings.profileSettings.board")}
            />
            <p className="text-xs text-muted-foreground">
              {t("settings.profileSettings.displayNameHelp")}
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="profile-email">{t("settings.profileSettings.email")}</Label>
            <Input
              id="profile-email"
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("settings.profileSettings.saving") : t("settings.profileSettings.saveProfile")}
            </Button>
          </div>
        </form>
