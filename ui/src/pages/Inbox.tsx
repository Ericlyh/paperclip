      ...
      <div className="flex items-start gap-2 sm:items-center">
          ...
          <span className="hidden sm:inline-flex h-4 w-4 shrink-0 items-center justify-center self-center">
            {showUnreadDot ? (
              <button
                ...
                aria-label={t("inbox.action.markAsRead")}
              >
                <span className={cn(
              ...
              <button
                ...
                aria-label={t("inbox.action.dismiss")}
              >
        ...
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
          ...
          >
            ...
            {isRetrying ? t("inbox.row.retrying") : t("inbox.row.retry")}
          </Button>
          {!showUnreadSlot && (
            <button
              type="button"
              ...
              aria-label={t("inbox.action.dismissShort")}
            >
              <X className="h-4 w-4" />
            </button>
          )}
...
export function Inbox() {
  ...
  const visibleMineIssues = useMemo(
    () => applyIssueFilters(mineIssues, issueFilters, currentUserId, true, liveIssueIds, {}, true, true, true),
    [mineIssues, issueFilters, currentUserId, liveIssueIds],
  );
  const visibleTouchedIssues = useMemo(
    () => applyIssueFilters(touchedIssues, issueFilters, currentUserId, true, liveIssueIds, {}, true, true, true),
  ...
  const activeIssueFilterCount = countActiveIssueFilters(issueFilters, true, true, true, true);
        ...
        <Tabs value={tab} onValueChange={(value) => navigate(`/inbox/${value}`)}>
          <PageTabBar
            items={[
              {
                value: "mine",
                label: t("inbox.tab.mine"),
              },
              {
                value: "recent",
                label: t("inbox.tab.recent"),
              },
              { value: "unread", label: t("inbox.tab.unread") },
              { value: "blocked", label: t("inbox.tab.blocked") },
              { value: "all", label: t("inbox.tab.all") },
            ]}
          />
        </Tabs>
                  ...
                  <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                    <DialogContent className="sm:max-w-md">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          {t("inbox.dialog.markAllAsReadDescription", { count: unreadIssueIds.length })}
                        </DialogDescription>
                      </DialogHeader>
                      <DialogFooter>
                        <Button variant="outline" onClick={() => setShowMarkAllReadConfirm(false)}>
                          {t("inbox.dialog.cancel")}
                        </Button>
                        <Button
                          onClick={() => {
