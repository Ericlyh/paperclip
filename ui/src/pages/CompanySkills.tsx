  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-48">
        <DropdownMenuLabel>{t("companySkills.source.label")}</DropdownMenuLabel>
        <DropdownMenuRadioGroup value={value} onValueChange={(next) => onChange(next as SourceFilter)}>
          {filters.map((filter) => (
            <DropdownMenuRadioItem key={filter} value={filter}>
              <span>{t(`companySkills.filter.type.${filter}`)}</span>
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="max-h-(--sz-calc-32) w-56 overflow-y-auto">
        <DropdownMenuLabel>{t("companySkills.filter.type.label")}</DropdownMenuLabel>
        <DropdownMenuRadioGroup value={kindFilter} onValueChange={(next) => onKindChange(next as "all" | "bundled" | "optional")}>
          <DropdownMenuRadioItem value="all">{t("companySkills.filter.type.all")}</DropdownMenuRadioItem>
          <DropdownMenuRadioItem value="bundled">{t("companySkills.filter.bundled")}</DropdownMenuRadioItem>
          <DropdownMenuRadioItem value="optional">{t("companySkills.filter.optional")}</DropdownMenuRadioItem>
        </DropdownMenuRadioGroup>
        <DropdownMenuSeparator />
        <DropdownMenuLabel>{t("companySkills.filter.category.label")}</DropdownMenuLabel>
        <DropdownMenuRadioGroup value={categoryFilter || "__all__"} onValueChange={(next) => onCategoryChange(next === "__all__" ? "" : next)}>
          <DropdownMenuRadioItem value="__all__">{t("companySkills.filter.allCategories")}</DropdownMenuRadioItem>
...
function TrustChip({ level }: { level: CompanySkillTrustLevel }) {
  ...
  const map = {
    markdown_only: {
      icon: ShieldCheck,
      label: t("companySkills.trustChip.markdownOnly"),
      tooltip: t("companySkills.trustChip.markdownOnlyTooltip"),
      className: "border-border bg-muted/40 text-muted-foreground",
    },
    assets: {
      icon: Folder,
      label: t("companySkills.trustChip.includesAssets"),
      tooltip: t("companySkills.trustChip.includesAssetsTooltip"),
      className: "border-cyan-500/30 bg-cyan-500/10 text-cyan-800 dark:text-cyan-200",
    },
    scripts_executables: {
      icon: AlertTriangle,
      label: t("companySkills.trustChip.includesScripts"),
      tooltip: t("companySkills.trustChip.includesScriptsTooltip"),
...
function CompatChip({ compatibility }: { compatibility: CompanySkillCompatibility }) {
  ...
  const map = {
    unknown: {
      icon: HelpCircle,
      label: t("companySkills.compatChip.unknownFormat"),
      tooltip: t("companySkills.compatChip.unknownFormatTooltip"),
      className: "border-yellow-500/40 bg-yellow-500/10 text-yellow-800 dark:text-yellow-200",
    },
    invalid: {
      icon: XOctagon,
      label: t("companySkills.compatChip.invalid"),
      tooltip: t("companySkills.compatChip.invalidTooltip"),
...
function ProvenanceBadge({ packageName, packageVersion }: { packageName: string | null; packageVersion: string | null }) {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent>{t("companySkills.provenanceBadge.tooltip")}</TooltipContent>
    </Tooltip>
  );
}
    ...
    >
      ...
      <div className="mt-auto pt-3">
        ...
        <div className="flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
          <span>{card.agentCount} {t("companySkills.card.agent", { count: card.agentCount })}</span>
          {card.starCount > 0 ? (
            <>
...
function CategoryNav({
  ...
  return (
    <nav className="flex flex-col gap-0.5 px-2">
      ...
      >
        <span>{t("companySkills.categoryNav.all")}</span>
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-border px-4 py-3">
          ...
          {availableSources.length > 1 ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm">
                  <span className="text-muted-foreground">{t("companySkills.toolbar.source")}</span>
                  <span className="ml-1.5 capitalize">
                    {sourceBadgeFilter === "all" ? t("companySkills.filter.type.all") : sourceMeta(sourceBadgeFilter as CompanySkillSourceBadge, null).label}
                  </span>
                  <ChevronDown className="ml-1 h-3.5 w-3.5" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end">
                <DropdownMenuRadioGroup value={sourceBadgeFilter} onValueChange={setSourceBadgeFilter}>
                  <DropdownMenuRadioItem value="all">{t("companySkills.filter.allSources")}</DropdownMenuRadioItem>
          ...
          <Button
            ...
            aria-label={t("companySkills.toolbar.scan")}
            title={t("companySkills.toolbar.scan")}
          >
          ...
          <Button asChild variant="outline" size="sm">
            <Link to="/skills/studio">
              ...
              {t("companySkills.toolbar.studio")}
            </Link>
          </Button>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onSelect={onCreate}>
                ...
                {t("companySkills.toolbar.createNewSkill")}
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={onBrowseCatalog}>
                <Boxes className="mr-2 h-4 w-4" />
                {t("companySkills.toolbar.browseCatalog")}
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={onImport}>
                <Globe className="mr-2 h-4 w-4" />
                {t("companySkills.toolbar.importFromPath")}
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={onImportFromProject}>
                <FolderSearch className="mr-2 h-4 w-4" />
                {t("companySkills.toolbar.importFromProject")}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          {folderResult && onFolderSelect ? (
            <div className="w-full md:hidden">
              <FolderChip
                result={folderResult}
                selection={folderSelection}
                allLabel={t("companySkills.folder.allSkills")}
          ...
          {onToggleSelectMode ? (
            <Button variant="ghost" size="sm" onClick={onToggleSelectMode}>
              {selectMode ? t("companySkills.toolbar.done") : t("companySkills.toolbar.select")}
            </Button>
          ) : null}
        </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="w-full justify-between">
                  <span className="capitalize">{activeCategory ?? t("companySkills.filter.allCategories")}</span>
              ...
              <DropdownMenuContent align="start" className="max-h-72 overflow-auto">
                ...
                >
                  <DropdownMenuRadioItem value="__all__">{t("companySkills.filter.allWithCount", { count: categoryTotal })}</DropdownMenuRadioItem>
                  {categories.map((category) => (
            ...
            <div className="mb-4 flex items-center justify-between gap-2">
              ...
              {activeProjectFolder && activeProjectId ? (
                ...
                >
                  ...
                  {t("companySkills.toolbar.refresh")}
                </Button>
              ) : null}
            </div>
          ) : null}
          ...
          {loading ? (
            ...
            <div className="py-12">
              <EmptyState
                ...
                message={
                  totalCount === 0
                    ? t("companySkills.empty.noSkills")
              ...
              {totalCount === 0 ? (
                <div className="mt-3 flex flex-col items-center gap-2">
                  <Button size="sm" onClick={onBrowseCatalog}>
                    <Boxes className="mr-1.5 h-3.5 w-3.5" /> {t("companySkills.empty.browseCatalog")}
                  </Button>
                  <Button size="sm" variant="ghost" onClick={onCreate}>
                    {t("companySkills.empty.createSkill")}
                  </Button>
                </div>
              ) : (search || activeCategory || sourceFilterActive) ? (
                <div className="mt-3 flex justify-center">
                  ...
                  >
                    {t("companySkills.empty.clearFilters")}
                  </Button>
                </div>
              ) : null}
            </div>
      ...
      {step === 0 ? (
        <div className="space-y-3">
          <Input
            ...
            placeholder={t("companySkills.wizard.skillName")}
            className="h-9"
          />
          <Input
            value={draft.slug}
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="truncate text-xs text-muted-foreground">{draft.tagline || t("companySkills.wizard.noTagline")}</div>
            </div>
          </div>
          <div>
          ...
          <div>
            <label className="mb-2 block text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.wizard.categoriesLabel")}</label>
            <Input
              value={categoryDraft}
              onChange={(event) => patchDraft({ categories: splitCategoryDraft(event.target.value) })}
              placeholder={t("companySkills.wizard.categoriesPlaceholder")}
              className="h-9"
            />
          </div>
        </div>
        ...
        <div className="space-y-4 text-sm">
          <div className="grid grid-cols-(--gtc-26) gap-y-2">
            <span className="text-muted-foreground">{t("companySkills.wizard.nameLabel")}</span>
            <span>{draft.name || t("companySkills.wizard.untitled")}</span>
            <span className="text-muted-foreground">{t("companySkills.wizard.slugLabel")}</span>
            <span className="font-mono">{effectiveSlug || "skill"}</span>
            <span className="text-muted-foreground">{t("companySkills.wizard.scopeLabel")}</span>
            <span>{draft.sharingScope === "private" ? t("companySkills.wizard.private") : t("companySkills.wizard.company")}</span>
            <span className="text-muted-foreground">{t("companySkills.wizard.categoriesLabel")}</span>
            <span>{draft.categories.length ? draft.categories.join(", ") : t("companySkills.wizard.none")}</span>
          </div>
          <div className="space-y-2">
            <label className="block text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.wizard.sharingLabel")}</label>
            <div className="grid gap-2 sm:grid-cols-3">
              {(["company", "private"] as const).map((scope) => (
                ...
                >
                  <span className="block font-medium">{scope === "company" ? t("companySkills.wizard.company") : t("companySkills.wizard.private")}</span>
                  <span className="mt-1 block text-xs text-muted-foreground">
                    {scope === "company" ? t("companySkills.wizard.companyDesc") : t("companySkills.wizard.privateDesc")}
                  </span>
                </button>
              ))}
              <button
              ...
              >
                <span className="block font-medium">{t("companySkills.wizard.publicLink")}</span>
                <span className="mt-1 block text-xs">{t("companySkills.wizard.comingLater")}</span>
              </button>
            </div>
          </div>
        </div>
  ...
  if (filtered.length === 0) {
    return (
      <div className="px-4 py-6 text-sm text-muted-foreground">
        {t("companySkills.catalog.noMatch")}
      </div>
    );
  }
  ...
  return (
    <div>
      {bundled.length > 0 && kindFilter !== "optional" ? (
        <div>
          <div className="border-b border-border bg-background px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            {t("companySkills.catalog.bundledWithCount", { count: bundled.length })}
          </div>
      ...
      {optional.length > 0 && kindFilter !== "bundled" ? (
        <div>
          <div className="border-b border-border bg-background px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            {t("companySkills.catalog.optionalWithCount", { count: optional.length })}
          </div>
      ...
      {installed.length > 0 ? (
        <div>
          <div className="border-b border-border bg-background px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            {t("companySkills.catalog.installedWithCount", { count: installed.length })}
          </div>
...
function CatalogDetailPane({
  ...
  const { t } = useTranslation();
  if (!skill) {
    return <EmptyState icon={Boxes} message={t("companySkills.catalog.selectSkill")} />;
  }
  ...
  if (skill.compatibility === "invalid") {
        ...
        <TooltipTrigger asChild>
          <span>
            <Button disabled>
              ...
              {t("companySkills.catalog.installSkill")}
            </Button>
          </span>
        </TooltipTrigger>
        <TooltipContent>{t("companySkills.catalog.cannotInstall")}</TooltipContent>
      </Tooltip>
    );
  } else if (!isInstalled) {
    cta = (
      <Button onClick={onInstall} disabled={loadingPrimaryAction}>
        ...
        {loadingPrimaryAction ? t("companySkills.catalog.preparing") : t("companySkills.catalog.installInOrg")}
      </Button>
    );
    ...
    cta = (
      <Button onClick={onUpdate} disabled={loadingPrimaryAction} className="border-amber-500/40 bg-amber-500/20 text-amber-900 dark:text-amber-100 hover:bg-amber-500/30">
        ...
        {t("companySkills.catalog.updateFromCatalog")}
      </Button>
    );
  } else {
    cta = (
      <Button variant="ghost" onClick={() => installedSkillId && onOpenInstalled(installedSkillId)}>
        ...
        {t("companySkills.catalog.installedOpenLibrary")}
      </Button>
    );
  }
    ...
    <div className="min-w-0">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Badge variant="outline" className="border-amber-500/40 bg-amber-500/10 text-(length:--text-micro) text-amber-800 dark:text-amber-200">
                  ...
                  {t("companySkills.catalog.updateAvailable")}
                </Badge>
              </TooltipTrigger>
              <TooltipContent>{t("companySkills.catalog.updateAvailableTooltip")}</TooltipContent>
            </Tooltip>
          ) : null}
          {skill.requires.length > 0 ? (
            <Badge variant="outline" className="border-border bg-muted/40 text-(length:--text-micro) text-muted-foreground">
              {t("companySkills.catalog.requires")}: {skill.requires.join(", ")}
            </Badge>
          ) : null}
          {skill.recommendedForRoles.length > 0 ? (
            <Badge variant="outline" className="border-border bg-muted/40 text-(length:--text-micro) text-muted-foreground">
              {t("companySkills.catalog.roles")}: {skill.recommendedForRoles.join(" · ")}
            </Badge>
          ) : null}
          {skill.tags.length > 0 ? (
            <Badge variant="outline" className="border-border bg-muted/40 text-(length:--text-micro) text-muted-foreground">
              {t("companySkills.catalog.tags")}: {skill.tags.join(" · ")}
            </Badge>
          ) : null}
        </div>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
          <span className="uppercase tracking-(--tracking-caps)">{t("companySkills.metadata.key")}</span>
          <span className="font-mono">{skill.key}</span>
          <span className="uppercase tracking-(--tracking-caps)">·</span>
          <span className="uppercase tracking-(--tracking-caps)">{t("companySkills.metadata.hash")}</span>
          <span className="font-mono">{skill.contentHash.slice(0, 24)}…</span>
          <CopyText
            text={skill.contentHash}
            copiedLabel={t("companySkills.metadata.copiedHash")}
            ariaLabel={t("companySkills.metadata.copyHash")}
            title={t("companySkills.metadata.copyHash")}
      ...
      <div className="min-h-(--sz-400px) px-5 py-5">
        {fileQuery.isLoading ? (
          ...
          <div className="text-sm text-muted-foreground">{t("companySkills.catalog.selectFile")}</div>
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => (!isPending ? onOpenChange(value) : null)}>
      <DialogContent className="sm:max-w-2xl" showCloseButton={!isPending}>
        <DialogHeader>
          <DialogTitle>
            {t(defaultAction === "update" ? "companySkills.installPreview.update" : defaultAction === "replace" ? "companySkills.installPreview.replace" : "companySkills.installPreview.install")} · {skill.name}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-4 text-sm">
          <div className="rounded-md border border-border p-3">
            <div className="grid grid-cols-(--gtc-26) gap-y-2 text-xs">
              <div className="text-muted-foreground">{t("companySkills.installPreview.trust")}</div>
              <div className="flex items-center gap-2">
                <TrustChip level={skill.trustLevel} />
                {skill.trustLevel === "markdown_only" ? (
                  <span className="text-muted-foreground">{t("companySkills.trustChip.safe")}</span>
                ) : skill.trustLevel === "scripts_executables" ? (
                  <span className="text-amber-800 dark:text-amber-200">{t("companySkills.trustChip.reviewRequired")}</span>
                ) : (
                  <span className="text-muted-foreground">{t("companySkills.trustChip.nonScriptAssets")}</span>
                )}
              </div>
              <div className="text-muted-foreground">{t("companySkills.installPreview.compatibility")}</div>
              <div className="flex items-center gap-2">
                {skill.compatibility === "compatible" ? (
                  <span className="inline-flex items-center gap-1 text-muted-foreground">
                    <Check className="h-3 w-3" aria-hidden="true" />
                    {t("companySkills.installPreview.compatible")}
                  </span>
                ) : (
                  <CompatChip compatibility={skill.compatibility} />
                )}
              </div>
              <div className="text-muted-foreground">{t("companySkills.installPreview.requires")}</div>
              <div className="text-foreground">{skill.requires.length === 0 ? t("companySkills.installPreview.none") : skill.requires.join(", ")}</div>
              <div className="text-muted-foreground">{t("companySkills.installPreview.roles")}</div>
              <div className="text-foreground">{skill.recommendedForRoles.length === 0 ? t("companySkills.installPreview.any") : skill.recommendedForRoles.join(" · ")}</div>
              <div className="text-muted-foreground">{t("companySkills.installPreview.provenance")}</div>
              <div className="min-w-0">
          ...
          <div className="rounded-md border border-border">
            <div className="border-b border-border px-3 py-2 text-xs uppercase tracking-wide text-muted-foreground">
              {t("companySkills.installPreview.files", { count: skill.files.length })}
            </div>
          ...
          {conflict ? (
            <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-3 text-xs text-amber-800 dark:text-amber-200">
              {t("companySkills.installPreview.conflict", { key: conflict.key, source: conflict.sourceLabel ?? conflict.sourceType, action: defaultAction === "update" ? t("companySkills.installPreview.overwriteCatalog") : t("companySkills.installPreview.replaceExisting") })}
            </div>
          ) : null}
          ...
          >
            ...
            {t("companySkills.installPreview.advanced")}
          </button>
          {advancedOpen ? (
            <div className="space-y-3 rounded-md border border-border p-3 text-xs">
              <div>
                <label className="mb-1 block uppercase tracking-wide text-muted-foreground">{t("companySkills.installPreview.slugOverride")}</label>
              ...
              <label className="flex items-center gap-2">
                ...
                <span>{t("companySkills.installPreview.forceReplace")}</span>
              </label>
            </div>
          ) : null}
        ...
        <DialogFooter>
          <Button variant="ghost" onClick={() => onOpenChange(false)} disabled={isPending}>
            {t("companySkills.installPreview.cancel")}
          </Button>
          <Button
...
function AttachAgentsPopover({
  ...
  return (
    <AgentMultiSelect
      ...
      pending={pending}
      triggerLabel={t("companySkills.agents.addToAgent")}
      ...
      headerContent={sortedVersions.length > 0 ? (
        <div className="mt-2 flex items-center gap-2 text-xs">
          <span className="shrink-0 text-muted-foreground">{t("companySkills.agents.version")}</span>
          <select
          ...
          >
            <option value="__latest__">{t("companySkills.agents.latest")}</option>
      ...
      emptyMessage={eligible.length === 0 ? t("companySkills.agents.noAgentsSupportSkills") : t("companySkills.agents.noAgents")}
      ...
      getDescription={(agent) => {
        const option = agent as AttachAgentOption;
        return `${option.adapterType}${option.required ? t("companySkills.agents.required") : ""}${!option.supportsSkills ? t("companySkills.agents.skillsNotSupported") : ""}`;
      }}
...
function SkillList({
  ...
  if (filteredSkills.length === 0) {
    if (sourceFilter !== "all" && skills.length > 0) {
      return (
        <div className="px-4 py-6 text-sm text-muted-foreground">
          {t("companySkills.skillList.noSkillsForSource", { source: SOURCE_FILTER_LABELS[sourceFilter].toLowerCase() })}{" "}
          <button type="button" className="text-foreground underline" onClick={onClearFilters}>
            {t("companySkills.skillList.clearFilter")}
          </button>
        </div>
      );
    }
      ...
      <DialogContent className="flex max-h-(--sz-85vh) w-full !max-w-(--pct-90) flex-col overflow-hidden">
        ...
        <div className="flex min-h-0 flex-1 gap-3">
          ...
          <div className="min-w-0 flex-1 overflow-auto rounded-md border border-border text-xs">
            {!right ? (
              <div className="p-6 text-center text-sm text-muted-foreground">{t("companySkills.versionDiff.selectToCompare")}</div>
            ) : left?.id === right.id ? (
...
function SkillLocationCard({
  ...
  return (
    <section>
      <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.location.label")}</div>
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        >
          ...
          {copied ? t("companySkills.location.copied") : t("companySkills.location.copyPath")}
        </Button>
        {onMove ? (
          <Button size="sm" variant="outline" onClick={onMove}>
            <FolderInput className="mr-1.5 h-3.5 w-3.5" />
            {t("companySkills.location.move")}
          </Button>
        ) : null}
      </div>
    </section>
      ...
      <div className="flex flex-wrap gap-1.5">
        {categories.map((tag) => (
          ...
          >
            ...
            <button
              type="button"
              aria-label={t("companySkills.tags.removeTag", { tag })}
              disabled={pending}
        ...
        {categories.length === 0 ? (
          <span className="text-xs text-muted-foreground">{t("companySkills.tags.empty")}</span>
        ) : null}
      </div>
      <Input
        value={draft}
        ...
        placeholder={t("companySkills.tags.placeholder")}
        disabled={pending}
...
export function SkillDetailPage({
  ...
  if (!detail) {
    return loading ? <PageSkeleton variant="detail" /> : <EmptyState icon={Boxes} message={t("companySkills.detail.skillNotFound")} />;
  }
    ...
    return (
      <div className="grid min-h-(--sz-560px) gap-0 lg:grid-cols-(--gtc-28)">
        <aside className="border-b border-border pb-3 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-3">
          <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.detail.files")}</div>
          <SkillTree
        ...
        <section className="min-w-0 lg:pl-5">
            ...
            <div className="flex items-center gap-2">
              {file?.markdown && !editMode ? (
                <div className="flex items-center border border-border">
                  ...
                  >
                    <span className="flex items-center gap-1.5"><Eye className="h-3.5 w-3.5" /> {t("companySkills.detail.view")}</span>
                  </button>
                  <button
                    className={cn("border-l border-border px-3 py-1.5 text-sm", viewMode === "code" ? "text-foreground" : "text-muted-foreground")}
                    onClick={() => setViewMode("code")}
                  >
                    <span className="flex items-center gap-1.5"><Code2 className="h-3.5 w-3.5" /> {t("companySkills.detail.code")}</span>
                  </button>
                </div>
              ) : null}
              {skill.editable && file?.editable ? (
                editMode ? (
                  <>
                    <Button variant="ghost" size="sm" onClick={() => setEditMode(false)} disabled={savePending}>{t("companySkills.detail.cancel")}</Button>
                    <Button size="sm" onClick={onSave} disabled={savePending}>
                      <Save className="mr-1.5 h-3.5 w-3.5" />
                      {savePending ? t("companySkills.detail.saving") : t("companySkills.detail.save")}
                    </Button>
                  </>
                ) : (
                  <Button variant="ghost" size="sm" onClick={() => setEditMode(true)}>
                    <Pencil className="mr-1.5 h-3.5 w-3.5" /> {t("companySkills.detail.edit")}
                  </Button>
                )
              ) : !skill.editable ? (
                <Button
                  ...
                  size="sm"
                  onClick={onFork}
                  title={skill.editableReason ?? t("companySkills.detail.forkToEdit")}
                >
                  <GitFork className="mr-1.5 h-3.5 w-3.5" />
                  {t("companySkills.detail.fork")}
                </Button>
              ) : null}
            </div>
          </div>
          {fileLoading ? (
            ...
            <div className="text-sm text-muted-foreground">{t("companySkills.detail.selectFile")}</div>
      ...
      <div className="space-y-6">
        <section>
          ...
          {fileLoading ? (
            ...
            <MarkdownBody softBreaks={false} linkIssueReferences={false}>{body || skill.description || t("companySkills.detail.noOverview")}</MarkdownBody>
          ) : (
            <p className="text-sm text-muted-foreground">{skill.description ?? t("companySkills.detail.noOverview")}</p>
          )}
        </section>
        <section className="grid min-w-0 gap-3 text-sm sm:grid-cols-2">
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("companySkills.detail.key")}</div>
            <div className="mt-1 truncate font-mono">{skill.key}</div>
          </div>
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("companySkills.detail.source")}</div>
            <div className="mt-1 min-w-0 [overflow-wrap:anywhere]">{sourceLocatorText ?? source.label}</div>
          </div>
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("companySkills.detail.version")}</div>
            <div className="mt-1">{versionLabel(skill.currentVersion ?? null)}</div>
          </div>
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("companySkills.detail.mode")}</div>
            <div className="mt-1 flex flex-wrap items-center gap-2">
              {skill.editable ? (
                t("companySkills.detail.editable")
              ) : (
                <>
                  <span>{t("companySkills.detail.readOnly")}</span>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div className="text-sm text-muted-foreground">
            {versionsLoading ? t("companySkills.versions.loading") : `${versions.length} ${versions.length === 1 ? t("companySkills.versions.version") : t("companySkills.versions.versions")}`}
          </div>
          <Button
            type="button"
            variant="outline"
          ...
          >
            <History className="mr-1.5 h-3.5 w-3.5" /> {t("companySkills.versions.compare")}
          </Button>
        </div>
        <div className="border-y border-border">
          {versionsLoading ? (
            ...
            <div className="py-6 text-sm text-muted-foreground">{t("companySkills.detail.noSavedVersions")}</div>
          ) : (
            sortedVersions.map((version) => (
              <div key={version.id} className="grid gap-2 border-b border-border px-0 py-3 text-sm last:border-b-0 sm:grid-cols-(--gtc-13)">
                ...
                >
                  {t("companySkills.versions.viewDiff")}
                </Button>
              </div>
            ))
          )}
    ...
    <div className="min-h-(--sz-calc-30)">
                ...
                {subtitleText ? (
                  <div className="mt-1 max-w-2xl">
                    ...
                    {descClamped ? (
                      ...
                      >
                        {descExpanded ? t("companySkills.detail.showLess") : t("companySkills.detail.viewAll")}
                      </button>
                    ) : null}
                  </div>
                ) : null}
          ...
          <div className="flex flex-wrap items-center justify-end gap-1">
            <Button variant="outline" size="sm" asChild>
              <Link to={resolvedStudioHref}>
                ...
                {t("companySkills.detail.openInStudio")}
              </Link>
            </Button>
            <div className="flex items-center overflow-hidden rounded-md border border-border">
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-muted-foreground">
                    ...
                    <span className="hidden sm:inline">{detail.attachedAgentCount === 1 ? t("companySkills.detail.install") : t("companySkills.detail.installs")}</span>
                  </span>
                </TooltipTrigger>
                <TooltipContent>{t("companySkills.detail.installsTooltip")}</TooltipContent>
              </Tooltip>
              <button
                type="button"
                ...
                title={detail.starredByCurrentActor ? t("companySkills.detail.unstar") : t("companySkills.detail.star")}
              >
                <Star className={cn("h-3.5 w-3.5", detail.starredByCurrentActor && "fill-current text-yellow-400")} />
                <span className="hidden sm:inline">{detail.starredByCurrentActor ? t("companySkills.detail.starred") : t("companySkills.detail.star")}</span>
              ...
              <button
                ...
                title={t("companySkills.detail.forkThisSkill")}
              >
                <GitFork className="h-3.5 w-3.5" />
                <span className="hidden sm:inline">{t("companySkills.detail.fork")}</span>
      ...
      <div className="grid gap-6 px-4 py-4 xl:grid-cols-(--gtc-29)">
              ...
              {SKILL_DETAIL_TABS.map((tab) => {
                ...
                return (
                  <TabsTrigger key={tab.value} value={tab.value} className="px-3">
                    ...
                    {t(`companySkills.detail.tabs.${tab.value}`)}
                  </TabsTrigger>
                );
              })}
            </TabsList>
        ...
        <aside className="min-w-0 space-y-6 border-t border-border pt-4 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0">
          ...
          />
          <section>
            <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.sidebar.agents")}</div>
            <div className="space-y-3">
              ...
              {detail.usedByAgents.length === 0 ? (
                <p className="text-xs text-muted-foreground">{t("companySkills.sidebar.noAgentsAttached")}</p>
              ) : (
                <div className="space-y-0.5">
                  ...
                  {detail.usedByAgents.slice(0, 3).map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {meta?.paused ? (
                          <Pause className="h-3 w-3 shrink-0 text-amber-500" aria-label={t("companySkills.agents.paused")} />
                        ) : null}
                      </Link>
                    );
                  })}
                  {detail.usedByAgents.length > 3 ? (
                    <p className="px-1.5 pt-0.5 text-xs text-muted-foreground">
                      {t("companySkills.sidebar.andMore", { count: detail.usedByAgents.length - 3 })}
                    </p>
                  ) : null}
                </div>
              )}
          ...
          {/* Provenance: where this skill came from, with org/path linked when
              ...
              (PAP-10907). */}
          <section>
            <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.sidebar.source")}</div>
            {githubSource ? (
          ...
          {detail.sourceType === "github" ? (
            <section>
              <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.sidebar.updates")}</div>
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Pin className="h-3.5 w-3.5 shrink-0" aria-label={t("companySkills.sidebar.pinnedRevisionAria")} />
                    </TooltipTrigger>
                    <TooltipContent>{t("companySkills.sidebar.pinnedRevision")}</TooltipContent>
                  </Tooltip>
                  <span className="truncate font-mono text-foreground">{currentPin ?? t("companySkills.sidebar.untracked")}</span>
                </div>
                <Button variant="outline" size="sm" className="w-full" onClick={onCheckUpdates} disabled={checkUpdatesPending || updateStatusLoading}>
                  ...
                  {t("companySkills.sidebar.checkForUpdates")}
                </Button>
                {updateStatus?.supported && updateStatus.hasUpdate ? (
                  <Button size="sm" className="w-full" onClick={onInstallUpdate} disabled={installUpdatePending}>
                    ...
                    {t("companySkills.sidebar.installUpdate", { pin: latestPin ?? "" })}
                  </Button>
                ) : updateStatus?.supported && !updateStatus.hasUpdate && !updateStatusLoading ? (
                  <p className="text-xs text-muted-foreground">{t("companySkills.sidebar.upToDate")}</p>
                ) : null}
              </div>
            </section>
          ) : null}
          ...
          <section>
            ...
            >
              ...
              <span className="flex-1">{t("companySkills.sidebar.settings")}</span>
            </button>
          </section>
        </aside>
      </div>
      ...
      {isDirty ? (
        <div className="fixed bottom-6 left-1/2 z-40 flex -translate-x-1/2 items-center gap-3 rounded-full border border-border bg-background/95 px-4 py-2 shadow-lg backdrop-blur">
          <span className="text-sm text-muted-foreground">{t("companySkills.detail.unsavedChanges")}</span>
          <Button
            variant="ghost"
            size="sm"
            onClick={() => {
          ...
          >
            {t("companySkills.detail.discard")}
          </Button>
          <Button size="sm" onClick={onSave} disabled={savePending}>
            <Save className="mr-1.5 h-3.5 w-3.5" />
            {savePending ? t("companySkills.detail.saving") : t("companySkills.detail.saveChanges")}
          </Button>
        </div>
      ) : null}
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("companySkills.settings.title")}</DialogTitle>
            <DialogDescription>{t("companySkills.settings.description", { name: detail.name })}</DialogDescription>
          </DialogHeader>
          <div className="space-y-5">
            <div className="space-y-1.5">
              <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.settings.categoriesLabel")}</label>
              <Input
                value={settingsCategoryDraft}
                onChange={(event) => setSettingsCategoryDraft(event.target.value)}
                placeholder={t("companySkills.settings.categoriesPlaceholder")}
                className="h-9"
                disabled={updateSettingsPending}
              />
              <p className="text-xs text-muted-foreground">{t("companySkills.settings.categoriesHint")}</p>
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.settings.sharingLabel")}</label>
              <select
              ...
              >
                <option value="company">{t("companySkills.settings.sharingCompany")}</option>
                <option value="private">{t("companySkills.settings.sharingPrivate")}</option>
              </select>
              <p className="text-xs text-muted-foreground">{t("companySkills.settings.publicLinkComing")}</p>
            </div>
            <div className="flex justify-end gap-2 border-t border-border pt-4">
              ...
              >
                {t("companySkills.settings.reset")}
              </Button>
              <Button
                type="button"
                size="sm"
              ...
              >
                <Save className="mr-1.5 h-3.5 w-3.5" />
                {updateSettingsPending ? t("companySkills.settings.saving") : t("companySkills.settings.saveSettings")}
              </Button>
            </div>
            {detail.editable ? (
              <div className="rounded-md border border-destructive/40 p-3">
                <div className="text-xs font-semibold uppercase tracking-wide text-destructive">{t("companySkills.settings.dangerZone")}</div>
                <div className="mt-2 flex items-center justify-between gap-3">
                  <p className="min-w-0 text-xs text-muted-foreground">{t("companySkills.settings.removeSkill")}</p>
                  <Button
                    variant="destructive"
                    size="sm"
                    className="shrink-0"
                    ...
                    title={detail.usedByAgents.length > 0 ? t("companySkills.settings.detachFirst") : undefined}
                  >
                    <Trash2 className="mr-1.5 h-3.5 w-3.5" />
                    {deletePending ? t("companySkills.settings.removing") : t("companySkills.settings.remove")}
                  </Button>
                </div>
              </div>
            ) : null}
    ...
    <div className="min-w-0">
      <div className="border-b border-border px-5 py-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            <Button variant="outline" size="sm" asChild>
              <Link to={skillStudioRoute(detail.id)}>
                <FlaskConical className="mr-1.5 h-3.5 w-3.5" />
                {t("companySkills.skillPane.openInStudio")}
              </Link>
            </Button>
            <Button
              variant="ghost"
            ...
            >
              <Trash2 className="mr-1.5 h-3.5 w-3.5" />
              {deletePending ? t("companySkills.skillPane.removing") : t("companySkills.skillPane.remove")}
            </Button>
            {detail.editable ? (
              <button
              ...
              >
                ...
                {editMode ? t("companySkills.skillPane.stopEditing") : t("companySkills.skillPane.edit")}
              </button>
            ) : (
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            <div className="flex min-w-0 items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("companySkills.skillPane.source")}</span>
                ...
                {detail.sourcePath && displaySourcePath ? (
                  <>
                    ...
                    <CopyText
                      ...
                      copiedLabel={t("companySkills.skillPane.copiedPath")}
                      ariaLabel={t("companySkills.skillPane.copySourcePath")}
                      title={t("companySkills.skillPane.copySourcePath")}
            ...
            {detail.sourceType === "github" && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("companySkills.skillPane.pin")}</span>
                <span className="font-mono text-xs">{currentPin ?? t("companySkills.skillPane.untracked")}</span>
                {updateStatus?.trackingRef && (
                  <span className="text-xs text-muted-foreground">{t("companySkills.skillPane.tracking")} {updateStatus.trackingRef}</span>
                )}
                <Button
                  variant="ghost"
                  size="sm"
                ...
                >
                  <RefreshCw className={cn("mr-1.5 h-3.5 w-3.5", (checkUpdatesPending || updateStatusLoading) && "animate-spin")} />
                  {t("companySkills.skillPane.checkForUpdates")}
                </Button>
                {updateStatus?.supported && updateStatus.hasUpdate && (
                  <Button
                    size="sm"
                  ...
                  >
                    ...
                    {t("companySkills.skillPane.installUpdate", { pin: latestPin ?? "" })}
                  </Button>
                )}
                {updateStatus?.supported && !updateStatus.hasUpdate && !updateStatusLoading && (
                  <span className="text-xs text-muted-foreground">{t("companySkills.skillPane.upToDate")}</span>
                )}
            ...
            <div className="flex items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("companySkills.skillPane.key")}</span>
              <span className="font-mono text-xs">{detail.key}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("companySkills.skillPane.mode")}</span>
              <span>{detail.editable ? t("companySkills.skillPane.editable") : t("companySkills.skillPane.readOnly")}</span>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("companySkills.skillPane.trust")}</span>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Badge variant="outline" className="border-violet-500/40 bg-violet-500/10 text-(length:--text-micro) text-violet-200">
                    ...
                    {t("companySkills.skillPane.locallyModified")}
                  </Badge>
                </TooltipTrigger>
                <TooltipContent>{t("companySkills.skillPane.locallyModifiedTooltip")}</TooltipContent>
              </Tooltip>
            ) : null}
            {(() => {
          ...
          <div className="flex flex-wrap items-start gap-x-3 gap-y-2">
            <div className="flex items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("companySkills.skillPane.usedBy")}</span>
              <AttachAgentsPopover
                agents={attachAgents}
            ...
            {usedBy.length === 0 ? (
              <span className="text-muted-foreground">{t("companySkills.skillPane.noAgentsAttached")}</span>
            ) : (
      ...
      <div className="min-h-(--sz-560px) px-5 py-5">
        {fileLoading ? (
          ...
          <div className="text-sm text-muted-foreground">{t("companySkills.skillPane.selectFile")}</div>
        ) : editMode && file.editable ? (
          file.markdown ? (
            <MarkdownEditor
              value={draft}
    ...
    setBreadcrumbs([
      ...
      ...(isStudioNew
        ...
        : activeDetail
          ...
          : routeSkillToken
            ? [{ label: t("companySkills.detail.breadcrumb") }]
            : []),
    ]);
  ...
  return (
    <>
      ...
      <Dialog open={deleteOpen} onOpenChange={closeDeleteDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("companySkills.dialog.removeSkill")}</DialogTitle>
            <DialogDescription>
              {t("companySkills.dialog.removeSkillDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <p>
              {deleteTargetDetail
                ? t("companySkills.dialog.aboutToRemove", { name: deleteTargetDetail.name })
                : t("companySkills.dialog.aboutToRemoveThis")}
            </p>
            {deleteTargetDetail?.usedByAgents?.length ? (
              <div className="rounded-md border border-border px-3 py-3 text-muted-foreground">
                {t("companySkills.dialog.currentlyUsedBy")} {deleteTargetDetail.usedByAgents.map((agent) => agent.name).join(", ")}.
              </div>
            ) : null}
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              <p className="text-muted-foreground">
                {t("companySkills.dialog.detachFirst")}
              </p>
            ) : null}
          </div>
          <DialogFooter>
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              ...
              <>
                ...
                >
                  {deleteSkill.isPending ? t("companySkills.dialog.removing") : t("companySkills.dialog.removeSkill")}
                </Button>
              </>
            )}
          </DialogFooter>
      ...
      <Dialog open={emptySourceHelpOpen} onOpenChange={setEmptySourceHelpOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("companySkills.dialog.addSource")}</DialogTitle>
            <DialogDescription>
              {t("companySkills.dialog.addSourceDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <a
            ...
            >
              <span>
                ...
                <span className="mt-1 block text-muted-foreground">
                  {t("companySkills.dialog.findCommands")}
                </span>
              </span>
            ...
            >
              <span>
                ...
                <span className="mt-1 block text-muted-foreground">
                  {t("companySkills.dialog.lookForRepos")}
                </span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
      ...
      <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("companySkills.import.title")}</DialogTitle>
            <DialogDescription>
              {t("companySkills.import.description")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="flex items-center gap-2 border-b border-border pb-2">
              <Input
                ...
                placeholder={t("companySkills.import.placeholder")}
              ...
              <Button size="sm" onClick={handleAddSkillSource} disabled={importSkill.isPending}>
                {importSkill.isPending ? <RefreshCw className="h-4 w-4 animate-spin" /> : t("companySkills.import.import")}
              </Button>
            </div>
            <a
              href="https://skills.sh"
            ...
            >
              <span>
                <span className="block font-medium">{t("companySkills.import.browseSkillsSh")}</span>
                <span className="mt-1 block text-muted-foreground">{t("companySkills.import.findCommands")}</span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
            <a
            ...
            >
              <span>
                <span className="block font-medium">{t("companySkills.import.searchGithub")}</span>
                <span className="mt-1 block text-muted-foreground">{t("companySkills.import.lookForRepos")}</span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
          </div>
      ...
      {isStudioNew ? (
          ...
          <div className="px-4 py-4">
            <div className="max-w-3xl">
              {studioForkFromId && studioForkDetailQuery.isLoading ? (
                ...
                <EmptyState icon={Boxes} message={t("companySkills.wizard.forkSourceNotFound")} />
              ) : (
                <NewSkillWizard
        ...
        <div className="min-h-(--sz-calc-30)">
          <div className="border-b border-border px-4 py-3">
            ...
            >
              <ChevronLeft className="h-4 w-4" />
              {t("companySkills.catalog.backToStore")}
            </Link>
          </div>
          {catalogListQuery.isLoading || catalogDetailQuery.isLoading ? (
            <PageSkeleton variant="detail" />
          ) : !selectedCatalogSkill ? (
            <EmptyState icon={Boxes} message={t("companySkills.catalog.notFound")} />
          ) : (
            <div className="grid gap-0 xl:grid-cols-(--gtc-30)">
              <aside className="border-b border-border px-3 py-4 xl:border-b-0 xl:border-r">
                <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("companySkills.catalog.files")}</div>
                <SkillTree
        ...
        <div className="min-h-(--sz-calc-30)">
          {skillsQuery.isLoading ? (
            ...
            <EmptyState icon={Boxes} message={t("companySkills.detail.skillNotFound")} />
          )}
        </div>
      )}
    </>
