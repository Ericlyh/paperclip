  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("connectionDialogs.catalogTitle", { name: connection.name })}</DialogTitle>
        </DialogHeader>
        {catalog.isLoading ? (
          <LoadingState />
        ) : catalog.error ? (
          ...
          <p className="py-6 text-sm text-muted-foreground">
            {t("connectionDialogs.catalogEmpty")}
          </p>
        ) : (
...
export function AddConnectionDialog({
  ...
  const create = useMutation({
    ...
    onError: (err) =>
      pushToast({
        title: t("connectionDialogs.toast.createFailed"),
  ...
  const probe = useMutation({
    ...
    onError: (err) =>
      pushToast({
        title: t("connectionDialogs.toast.probeFailed"),
        body: err instanceof ApiError ? err.message : String(err),
        tone: "error",
      }),
  });
  ...
  const activate = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t("connectionDialogs.toast.connectionActivated"), tone: "success" });
      onClose();
    },
    onError: (err) =>
      pushToast({
        title: t("connectionDialogs.toast.activationFailed"),
        body: err instanceof ApiError ? err.message : String(err),
        tone: "error",
      }),
  });
  ...
  const locked = !!draft;
  const inferredType = transport === "mcp_remote" ? t("connectionDialogs.transportRemoteHttp") : t("connectionDialogs.transportLocalStdio");

  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>{t("connectionDialogs.addApplicationTitle")}</DialogTitle>
          <DialogDescription>
            {t("connectionDialogs.addApplicationDescription")}
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-3">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className={step === 1 ? "font-medium text-foreground" : ""}>{t("connectionDialogs.step.application")}</span>
            <span>/</span>
            <span className={step === 2 ? "font-medium text-foreground" : ""}>{t("connectionDialogs.step.connection")}</span>
          </div>

          {step === 1 && !locked ? (
            <>
              <div className="space-y-1.5">
                <Label>{t("connectionDialogs.applicationLabel")}</Label>
                <Select value={applicationMode} onValueChange={(v) => setApplicationMode(v as "existing" | "new")}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="existing">{t("connectionDialogs.useExistingApplication")}</SelectItem>
                    <SelectItem value="new">{t("connectionDialogs.createNewApplication")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {applicationMode === "existing" ? (
                <div className="space-y-1.5">
                  <Label>{t("connectionDialogs.existingApplicationLabel")}</Label>
                  <Select value={applicationId} onValueChange={setApplicationId}>
                    <SelectTrigger>
                      <SelectValue placeholder={t("connectionDialogs.selectApplicationPlaceholder")} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="app-name">{t("connectionDialogs.newApplicationNameLabel")}</Label>
                  <Input
                    id="app-name"
                    value={applicationName}
                    onChange={(e) => setApplicationName(e.target.value)}
                    placeholder={t("connectionDialogs.newApplicationNamePlaceholder")}
                  />
                  <p className="text-xs text-muted-foreground">
                    {t("connectionDialogs.applicationTypeInferred")}
                  </p>
                </div>
              )}
            </>
            ...
            <>
              {applicationMode === "new" ? (
                <div className="rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
                  <span className="font-medium text-foreground">{applicationName.trim()}</span>{" "}
                  {t("connectionDialogs.willBeCreatedAs", { type: inferredType })}
                </div>
              ) : null}

              <div className="space-y-1.5">
                <Label htmlFor="conn-name">{t("connectionDialogs.connectionNameLabel")}</Label>
                <Input
                  id="conn-name"
                  value={name}
              ...
              <div className="space-y-1.5">
                <Label>{t("connectionDialogs.transportLabel")}</Label>
                <Select
                  value={transport}
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="mcp_remote">{t("connectionDialogs.transportRemoteHttp")}</SelectItem>
                    <SelectItem value="local_stdio">{t("connectionDialogs.transportLocalStdio")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {transport === "mcp_remote" ? (
                <div className="space-y-1.5">
                  <Label htmlFor="conn-url">{t("connectionDialogs.endpointUrlLabel")}</Label>
                  <Input
                    id="conn-url"
                    value={endpointUrl}
                ...
                <div className="space-y-1.5">
                  <Label>{t("connectionDialogs.commandTemplateLabel")}</Label>
                  <Select value={templateId} onValueChange={setTemplateId} disabled={locked}>
                    <SelectTrigger>
                      <SelectValue placeholder={t("connectionDialogs.commandTemplatePlaceholder")} />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("connectionDialogs.commandTemplateHint")}
                  </p>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                <Label>{t("connectionDialogs.credentialReferencesLabel")}</Label>
                {creds.length > 0 ? (
                      ...
                      >
                        ...
                        {!locked ? (
                          <button
                            ...
                            aria-label={t("connectionDialogs.removeCredentialAria", { name: secretName(c.secretId) })}
                          >
                ...
                {!locked ? (
                  <>
                    <div className="flex items-end gap-2">
                      <div className="flex-1 space-y-1">
                        <Select value={pendingSecretId} onValueChange={setPendingSecretId}>
                          <SelectTrigger>
                            <SelectValue placeholder={t("connectionDialogs.selectVaultSecretPlaceholder")} />
                          </SelectTrigger>
                          <SelectContent>
                            {(secrets.data ?? []).map((s) => (
                      ...
                      <Input
                        ...
                        placeholder={t("connectionDialogs.headerPlaceholder")}
                        className="w-32"
                        aria-label={t("connectionDialogs.headerNameAria")}
                      />
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("connectionDialogs.freeTextSecretsRejected")}
                      <span className="font-mono"> {t("connectionDialogs.vaultReference")}</span> {t("connectionDialogs.resolvesAtGateway")}
                    </p>
                  </>
                ) : null}
              </div>
          ...
          {locked ? (
            probe.isPending ? (
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <LoadingState label={t("connectionDialogs.probingLabel")} />
              </div>
            ) : probe.isError ? (
              ...
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <div className="flex items-center gap-2 text-sm">
                  <span className="font-medium text-foreground">{t("connectionDialogs.probeResult")}</span>
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{t("connectionDialogs.toolsDiscovered")}</p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold tabular-nums text-foreground">
                      {probeResult.latencyMs != null ? `${probeResult.latencyMs}ms` : "—"}
                    </p>
                    <p className="text-xs text-muted-foreground">{t("connectionDialogs.probeLatency")}</p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold tabular-nums text-foreground">
                      {probeResult.quarantinedCount}
                    </p>
                    <p className="text-xs text-muted-foreground">{t("connectionDialogs.quarantined")}</p>
                  </div>
                </div>
                ...
                <p className="mt-2 text-(length:--text-micro) text-muted-foreground">
                  {t("connectionDialogs.probeLatencyFootnote")}
                </p>
              </div>
            ) : null
          ) : null}
          ...
          {step === 1 && !locked ? (
            ...
            <>
              ...
              <Button disabled={!canCreate} onClick={() => create.mutate()}>
                {create.isPending ? t("connectionDialogs.creatingDraft") : t("connectionDialogs.createAndProbe")}
              </Button>
            </>
          ) : (
            <>
              <Button variant="outline" disabled={probe.isPending} onClick={() => draft && probe.mutate(draft.id)}>
                ...
                {probe.isPending ? t("connectionDialogs.probing") : t("connectionDialogs.reprobe")}
              </Button>
              <Button disabled={activate.isPending || probe.isPending} onClick={() => draft && activate.mutate(draft.id)}>
                {activate.isPending ? t("connectionDialogs.activating") : t("connectionDialogs.activate")}
              </Button>
            </>
          )}
        </DialogFooter>
