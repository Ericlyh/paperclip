...
function formatOwner(gateway: ToolMcpGatewayWithTokens, agentNames: Map<string, string>, t: TFunction) {
  if (gateway.agentId) {
    const name = agentNames.get(gateway.agentId);
    return name ?? t("gatewaysTab.card.ownerAgent", { id: shortId(gateway.agentId) ?? "" });
  }
  if (gateway.createdByAgentId) {
    const name = agentNames.get(gateway.createdByAgentId);
    return name ?? t("gatewaysTab.card.ownerAgent", { id: shortId(gateway.createdByAgentId) ?? "" });
  }
  if (gateway.createdByUserId) {
    return t("gatewaysTab.card.ownerBoardUser", { id: shortId(gateway.createdByUserId) ?? "" });
  }
  return t("gatewaysTab.card.ownerBoard");
}
...
function formatScope(
  ...
  if (gateway.contextScopeType !== "none" && gateway.contextScopeId) {
    if (gateway.contextScopeType === "project") {
      const name = projectNames.get(gateway.contextScopeId);
      return name ?? t("gatewaysTab.card.scopeProject", { name: shortId(gateway.contextScopeId) ?? "" });
    }
    if (gateway.contextScopeType === "agent") {
      const name = agentNames.get(gateway.contextScopeId);
      return name ?? t("gatewaysTab.card.scopeAgent", { name: shortId(gateway.contextScopeId) ?? "" });
    }
    return t("gatewaysTab.card.scopeRaw", { type: gateway.contextScopeType, id: shortId(gateway.contextScopeId) ?? "" });
  }
  if (gateway.projectId) {
    const name = projectNames.get(gateway.projectId);
    return name ?? t("gatewaysTab.card.scopeProject", { name: shortId(gateway.projectId) ?? "" });
  }
  if (gateway.issueId) return t("gatewaysTab.card.scopeIssue", { id: shortId(gateway.issueId) ?? "" });
  if (gateway.agentId) {
    const name = agentNames.get(gateway.agentId);
    return name ?? t("gatewaysTab.card.scopeAgent", { name: shortId(gateway.agentId) ?? "" });
  }
  return t("gatewaysTab.card.scopeCompany");
}

function formatAllowedTools(profile: ToolProfileWithDetails | undefined, t: TFunction) {
  if (!profile) return t("gatewaysTab.allowedTools.unavailable");
  const allowed = profile.summary.allowedToolCount;
  if (profile.summary.accessMode === "all_except") {
    const count = Math.max(profile.summary.totalToolCount - profile.summary.excludedToolCount, 0);
    return t("gatewaysTab.allowedTools.allowed", { count });
  }
  return allowed === 0
    ? t("gatewaysTab.allowedTools.noAllowed")
    : t("gatewaysTab.allowedTools.allowed", { count: allowed });
}
...
export function GatewaysTab({ companyId }: { companyId: string }) {
  ...
  const createGatewayMutation = useMutation({
    ...
    onSuccess: async (gateway) => {
      ...
      setCreating(false);
      pushToast({ title: t("gatewaysTab.toast.gatewayCreated"), body: gateway.name, tone: "success" });
      await invalidateGateways();
    },
    onError: (error) => {
      pushToast({ title: t("gatewaysTab.toast.gatewayNotCreated"), body: error instanceof Error ? error.message : String(error), tone: "error" });
    },
  });
  ...
  const createTokenMutation = useMutation({
    ...
    onSuccess: async (token) => {
      ...
      pushToast({
        title: t("gatewaysTab.toast.tokenIssued"),
        body: t("gatewaysTab.toast.tokenIssuedBody", { name: token.name }),
        tone: "success",
      });
      await invalidateGateways();
    },
    onError: (error) => {
      pushToast({ title: t("gatewaysTab.toast.tokenNotIssued"), body: error instanceof Error ? error.message : String(error), tone: "error" });
    },
  });
  ...
  const revokeTokenMutation = useMutation({
    ...
    onSuccess: async (token) => {
      ...
      pushToast({ title: t("gatewaysTab.toast.tokenRevoked"), body: token.name, tone: "success" });
      await invalidateGateways();
    },
    onError: (error) => {
      pushToast({ title: t("gatewaysTab.toast.tokenNotRevoked"), body: error instanceof Error ? error.message : String(error), tone: "error" });
    },
  });
  ...
  async function copyText(value: string, label: string) {
    try {
      ...
      pushToast({ title: t("gatewaysTab.toast.copiedToClipboard"), body: label, tone: "success" });
    } catch (error) {
      pushToast({
        title: t("gatewaysTab.toast.copyFailed"),
        body: error instanceof Error ? error.message : t("gatewaysTab.toast.copyFailedUnavailable"),
        tone: "error",
      });
    }
  }
  ...
  function submitCreateGateway(event: FormEvent<HTMLFormElement>) {
    ...
    if (!createDraft.profileId) {
      pushToast({
        title: t("gatewaysTab.toast.pickProfile"),
        body: t("gatewaysTab.toast.pickProfileBody"),
        tone: "warn",
      });
      return;
    }
  ...
  function submitCreateToken(event: FormEvent<HTMLFormElement>, gatewayId: string) {
    ...
    if (draft.allowedActions.length === 0) {
      pushToast({
        title: t("gatewaysTab.toast.pickTokenActions"),
        body: t("gatewaysTab.toast.pickTokenActionsBody"),
        tone: "warn",
      });
      return;
    }
  ...
  if (gatewaysQuery.isLoading) return <LoadingState label={t("gatewaysTab.loading.gateways")} />;
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <ToolsPageHeader
          title={t("gatewaysTab.title")}
          description={t("gatewaysTab.description")}
        />
        <Button
          type="button"
          size="sm"
        ...
        <form className="space-y-3 rounded-md border border-border p-4" onSubmit={submitCreateGateway}>
          <div className="grid gap-3 md:grid-cols-(--gtc-60)">
            <label className="space-y-1.5 text-sm">
              <span className="text-xs font-medium text-muted-foreground">{t("gatewaysTab.create.nameLabel")}</span>
              <input
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                value={createDraft.name}
                onChange={(event) => setCreateDraft((current) => ({ ...current, name: event.target.value }))}
                placeholder={t("gatewaysTab.create.namePlaceholder")}
                required
              />
            </label>
            <label className="space-y-1.5 text-sm">
              <span className="text-xs font-medium text-muted-foreground">{t("gatewaysTab.create.profileLabel")}</span>
              <select
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              ...
              >
                <option value="" disabled>
                  {profileLoading ? t("gatewaysTab.create.profileLoading") : t("gatewaysTab.create.profilePlaceholder")}
                </option>
          ...
          <label className="space-y-1.5 text-sm">
            <span className="text-xs font-medium text-muted-foreground">{t("gatewaysTab.create.descriptionLabel")}</span>
            <textarea
              className="min-h-20 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              value={createDraft.description}
              onChange={(event) => setCreateDraft((current) => ({ ...current, description: event.target.value }))}
              placeholder={t("gatewaysTab.create.descriptionPlaceholder")}
            />
          </label>
          {activeProfiles.length === 0 && !profileLoading ? (
            <p className="text-xs text-muted-foreground">{t("gatewaysTab.create.profileMissing")}</p>
          ) : null}
          <div className="flex flex-wrap justify-end gap-2">
            ...
            <Button type="submit" size="sm" disabled={createDisabled || !createDraft.name.trim() || !createDraft.profileId}>
              {createGatewayMutation.isPending ? t("gatewaysTab.create.creatingButton") : t("gatewaysTab.create.createButton")}
            </Button>
          </div>
        </form>
      ) : null}
              ...
              <section key={gateway.id} className="space-y-4 p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button type="button" variant="outline" size="sm" onClick={() => void copyText(endpoint, t("gatewaysTab.card.endpointLabel"))}>
                      <Copy className="mr-1.5 h-3.5 w-3.5" />
                      {t("gatewaysTab.card.copyEndpoint")}
                    </Button>
                    <Button type="button" variant="outline" size="sm" onClick={() => startIssuing(gateway.id)}>
                      <KeyRound className="mr-1.5 h-3.5 w-3.5" />
                      {t("gatewaysTab.card.issueToken")}
                    </Button>
                  </div>
                </div>
                ...
                <dl className="grid gap-x-4 gap-y-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("gatewaysTab.card.owner")}</dt>
                    <dd className="mt-0.5 text-foreground">{formatOwner(gateway, agentNames, t)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("gatewaysTab.card.scope")}</dt>
                    <dd className="mt-0.5 text-foreground">{formatScope(gateway, projectNames, agentNames, t)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("gatewaysTab.card.allowedTools")}</dt>
                    <dd className="mt-0.5 text-foreground">
                      {profile
                        ? t("gatewaysTab.card.allowedToolsVia", { summary: formatAllowedTools(profile, t), name: profile.name })
                        : t("gatewaysTab.card.profileMissing", { id: shortId(gateway.profileId) ?? "" })}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("gatewaysTab.card.lastActivity")}</dt>
                    <dd className="mt-0.5 text-foreground">
                      {lastActivity ? <RelativeTime value={lastActivity} /> : t("gatewaysTab.card.neverUsed")}
                    </dd>
                  </div>
                </dl>
                ...
                {issuingGatewayId === gateway.id ? (
                  <form className="space-y-3 rounded-md border border-border p-3" onSubmit={(event) => submitCreateToken(event, gateway.id)}>
                    <div className="grid gap-3 md:grid-cols-2">
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("gatewaysTab.issueToken.nameLabel")}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          ...
                          placeholder={t("gatewaysTab.issueToken.namePlaceholder")}
                          required
                        />
                      </label>
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("gatewaysTab.issueToken.clientLabel")}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          ...
                          placeholder={t("gatewaysTab.issueToken.clientPlaceholder")}
                          required
                        />
                      </label>
                    </div>
                    <div className="grid gap-3 md:grid-cols-[1fr_auto]">
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("gatewaysTab.issueToken.ownerNoteLabel")}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          ...
                          placeholder={t("gatewaysTab.issueToken.ownerNotePlaceholder")}
                          required
                        />
                      </label>
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("gatewaysTab.issueToken.expiresLabel")}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          type="date"
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      ...
                      >
                        {createTokenMutation.isPending ? t("gatewaysTab.issueToken.issuingButton") : t("gatewaysTab.issueToken.issueButton")}
                      </Button>
                    </div>
                  </form>
                ) : null}
                ...
                {createdToken ? (
                  <div className="space-y-2 rounded-md border border-border bg-muted/40 p-3 text-sm">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="font-medium text-foreground">{t("gatewaysTab.newToken.heading", { name: createdToken.name })}</div>
                      <Button type="button" variant="outline" size="sm" onClick={() => void copyText(createdToken.token, t("gatewaysTab.newToken.bearerLabel"))}>
                        <Copy className="mr-1.5 h-3.5 w-3.5" />
                        {t("gatewaysTab.newToken.copyToken")}
                      </Button>
                    </div>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div>
                    <div className="mb-1.5 flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
                      ...
                      {t("gatewaysTab.tokens.heading")}
                    </div>
                    <div className="space-y-1 text-sm">
                      {gateway.tokens.length === 0 ? (
                        <p className="text-muted-foreground">{t("gatewaysTab.tokens.empty")}</p>
                      ) : (
                            ...
                            <div key={token.id} className="space-y-1 py-1">
                              <div className="flex items-center justify-between gap-3">
                                ...
                                <div className="flex shrink-0 items-center gap-2">
                                  <span className="text-xs text-muted-foreground">
                                    {token.revokedAt ? (
                                      <>
                                        {t("gatewaysTab.tokens.revoked")} <RelativeTime value={token.revokedAt} />
                                      </>
                                    ) : token.expiresAt ? (
                                      <>
                                        {t("gatewaysTab.tokens.expires")} <RelativeTime value={token.expiresAt} />
                                      </>
                                    ) : (
                                      t("gatewaysTab.tokens.noExpiry")
                                    )}
                                  </span>
                                  {!revoked ? (
                                    <Button
                                      ...
                                      aria-label={t("gatewaysTab.tokens.revokeAria", { name: token.name })}
                                    >
                                      <RotateCcw className="mr-1 h-3.5 w-3.5" />
                                      {t("gatewaysTab.tokens.revokeAction")}
                                    </Button>
                                  ) : null}
                                </div>
                              </div>
                              {confirming ? (
                                <div className="flex flex-wrap items-center justify-end gap-2 text-xs text-muted-foreground">
                                  <span>{t("gatewaysTab.tokens.confirmRevokePrompt")}</span>
                                  <Button
                                    type="button"
                                    variant="ghost"
                                    size="sm"
                  ...
                  <div>
                    <div className="mb-1.5 text-xs font-medium text-muted-foreground">{t("gatewaysTab.snippets.heading")}</div>
                    <div className="space-y-1 text-sm">
                      {snippets.length === 0 ? (
                        <p className="text-muted-foreground">{t("gatewaysTab.snippets.empty")}</p>
                      ) : (
                          ...
                          <details key={snippet.client} className="rounded px-2 py-1 open:bg-muted/40">
                            <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-left">
                              ...
                              <Button
                                ...
                                onClick={(event) => {
                                  event.preventDefault();
                                  void copyText(formatSnippetConfig(snippet.config), t("gatewaysTab.snippets.snippetLabel", { label: snippet.label }));
                                }}
                              >
                                <Copy className="mr-1 h-3.5 w-3.5" />
                                {t("gatewaysTab.snippets.copy")}
                              </Button>
                            </summary>
