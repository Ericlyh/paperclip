...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("instanceSettings.breadcrumb"), href: "/company/settings/instance/general" },
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.errors.updateHeartbeat"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("instanceSettings.errors.unknownError");
        throw new Error(
          failures.length === 1
            ? t("instanceSettings.errors.disableOne", { detail })
            : t("instanceSettings.errors.disableMany", {
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.errors.disableAll"));
    },
  });
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("instanceSettings.errors.loadFailed")}
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("instanceSettings.stats.active")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("instanceSettings.stats.disabled")}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("instanceSettings.stats.company_one") : t("instanceSettings.stats.company_other")}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              const noun = enabledCount === 1 ? t("instanceSettings.confirm.agent_one") : t("instanceSettings.confirm.agent_other");
              if (!window.confirm(t("instanceSettings.confirm.disableAll", { count: enabledCount, noun }))) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? t("instanceSettings.actions.disabling") : t("instanceSettings.actions.disableAll")}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instanceSettings.empty")}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("instanceSettings.status.on") : t("instanceSettings.status.off")}
                        </Badge>
                        <Link
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            ...
                            title={t("instanceSettings.actions.fullConfig")}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("instanceSettings.actions.disableTimer") : t("instanceSettings.actions.enableTimer")}
                          </Button>
                        </span>
                      </div>
                    );
