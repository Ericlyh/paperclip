...
//
// The strings themselves live in i18n under the `toolInstalls.*` namespace so
// every surface still speaks the same language. These constants centralize the
// *keys*; call `t()` at the render site (a module-level const has no hook scope).

export const TOOL_INSTALL_COPY = {
  installInfoNotice: "toolInstalls.installInfoNotice",
  installAllWarning: "toolInstalls.installAllWarning",
  autoExtendNotice: "toolInstalls.autoExtendNotice",
  installedHint: "toolInstalls.installedHint",
} as const;
